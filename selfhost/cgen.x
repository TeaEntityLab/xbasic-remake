...
PRINT "static char* xb_str_float(double v) { char buf[32]; snprintf(buf, 32, " + CHR$(34) + "%.17g" + CHR$(34) + ", v); return xb_from_cstr(buf); }"
...
PRINT "static char* xb_binb(int v) { char buf[35]; int n = 0; if (v == 0) { buf[0] = '0'; buf[1] = 'b'; buf[2] = '0'; buf[3] = 0; return xb_from_cstr(buf); } int t = v; while (t) { n++; t >>= 1; } buf[0] = '0'; buf[1] = 'b'; buf[n + 2] = 0; t = v; while (t) { buf[--n + 2] = (t & 1) ? '1' : '0'; t >>= 1; } return xb_from_cstr(buf); }"
...
PRINT "static char* xb_bin2(int v, int d) { char buf[33]; int n = 0; int t = v; while (t) { n++; t >>= 1; } if (n < d) n = d; if (v == 0) { if (d > 0) n = d; else { buf[0] = '0'; buf[1] = 0; return xb_from_cstr(buf); } } buf[n] = 0; t = v; int pos = n - 1; while (pos >= 0) { buf[pos--] = (t & 1) ? '1' : '0'; t >>= 1; } return xb_from_cstr(buf); }"
...
PRINT "static char* xb_binb2(int v, int d) { char buf[35]; int n = 0; int t = v; while (t) { n++; t >>= 1; } if (n < d) n = d; if (v == 0 && d == 0) { buf[0] = '0'; buf[1] = 'b'; buf[2] = '0'; buf[3] = 0; return xb_from_cstr(buf); } if (v == 0) n = d; buf[0] = '0'; buf[1] = 'b'; buf[n + 2] = 0; t = v; int pos = n + 1; while (pos > 1) { buf[pos--] = (t & 1) ? '1' : '0'; t >>= 1; } return xb_from_cstr(buf); }"
...
PRINT "  else { char buf[32];"
PRINT "    if (xb_data_tag[xb_data_pos] == 0) snprintf(buf, 32, " + CHR$(34) + "%d" + CHR$(34) + ", xb_data_int[xb_data_pos]);"
PRINT "    else snprintf(buf, 32, " + CHR$(34) + "%.17g" + CHR$(34) + ", xb_data_float[xb_data_pos]);"
