...
PRINT "#ifdef _WIN32"
PRINT "    (void)nonblock;"
PRINT "    if (base == 0 || base == 0x10) f = fopen(name, \"rb\");"
PRINT "    else if (base == 1 || base == 3 || base == 4) f = fopen(name, \"w+b\");"
PRINT "    else if (base == 2 || base == 0x20 || base == 0x30) { f = fopen(name, \"r+b\"); if (!f) f = fopen(name, \"w+b\"); }"
...
PRINT "    if (fd >= 0) { f = fdopen(fd, access == 0 ? \"rb\" : (access == 1 ? \"wb\" : \"r+b\")); if (!f) close(fd); }"
PRINT "#endif"
PRINT "    if (!f) return -1;"
