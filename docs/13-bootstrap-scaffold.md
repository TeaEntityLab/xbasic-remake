...
1. **Stage 0 — Rust host:** Rust owns lexer, parser, codegen boundary, runtime ABI, exception maps, linker driver, GUI shell, and deterministic tests.
2. **Stage 1 — utility self-hosting:** low-level utilities and library surfaces are rewritten in XBasic using the Rust runtime as the execution substrate.
3. **Stage 2 — compiler self-hosting:** a compiler subset is reimplemented in XBasic after the Rust frontend/backend has stable tests and object-code output.
4. **Stage 3 — optional backend split:** LLVM remains the primary AOT backend; Cranelift can be revisited as a Win64/Linux/macOS debug backend only after the language semantics are stable.
