...
| CGEN-BYREF-ARG | C backend | `ByRef(inner)⇒emit_expr(inner)` passes the value, so a by-ref arg to an *array/pointer* param (`double *`) gets a `double` — int is masked by `-Wno-int-conversion`, but **float `double`→`double *` is a hard cc error** (7 XBSourceLib libs). **Two reactive attempts both regressed (2026-08-20), reverted:** (1) a blanket `&` on scalar by-ref broke msc's `@value#`→scalar-`double` param ("remove &"); (2) the callee-param-driven `&`-only-for-array-params version broke msc differently — the scalar-vs-array classifier (`is_dyn_array‖is_dual_use‖is_undimmed_array‖array_dims`) flagged `term$` as an array and emitted `xb_str_term_s_arr`, an *undeclared* `_arr` facet. Root: no reliable per-name array-vs-scalar signal, and a name flagged array may have no declared array facet. Needs a **ground-up by-ref redesign** (reliable param/arg array classification + real write-back), not incremental patching. **No corpus/demo impact** (corpus has 0 by-ref; no demo has float-by-ref since 114/114 compile) | XBSourceLib `ary`/`fgr`/`vgr`/merge (7 CFAIL) | feature |
