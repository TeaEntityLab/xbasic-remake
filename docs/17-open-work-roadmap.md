...
> test --workspace --exclude xb-ide` = **219 passed / 0 failed**; `cgen_cemitter_sync`
> **14/14** (byte-identity intact; +computed-GOTO +AT-write +pos/SIZE +giant-literal +float-arith +multidim +undeclared-local-hoist +non-Main-entry +unknown-call-drop); +4 interp Giant-context fixes; +cgen.x float-arith, multi-dim, hoisting, entry-point & unknown-call fixes (self-hosted demo faithfulness 1→42); native bootstrap fixed point **intact**; LLVM
...
| CGEN-SELFHOST-PARITY | self-hosted cgen.x | **The demo sweep/differential measure the Rust `CEmitter` (`--compile`), not cgen.x.** cgen.x's *true* demo faithfulness (build native cgen from cgen.x, then IR→cgen→cc→run vs interp over 114 demos) was **faithful=1** — cgen.x was a **minimal self-hosting seed** (selfhost subset: `Main` entry, every local DIM'd, no unknown builtins/shared arrays). Now **faithful=42, diverge=1 (amerge), cc-fail=70**. Fixed (all byte-neutral on selfhost+v0.1 → sync 14/14, bootstrap intact): **(0) ~~entry point~~ ✅ (`8484a72`)** — called only `xb_user_Main()`; all demos use `FUNCTION Entry` → called nothing → empty; now `Main` else first parameterless fn (`entry_or_first`). **(1) ~~undeclared-local hoisting~~ ✅ (`d98c635`)** — mirrors `emit_hoisted_scalars`. **(2) ~~unknown-call drop~~ ✅ (`b4e9413`)** — external `Xui*`/`Xgr*`/`Xst*` calls (not user fn, not known builtin) now no-op (stmt) / `0` (expr) like `is_unknown_call`, was the ~85-demo wall. Locked by `cemitter_and_cgen_agree_on_{undeclared_local,non_main_entry,unknown_call}`. Remaining cc-fails (70): ~45 misc (need triage), 16 SHARED-array (CGEN-SHARED-ARR-SELFHOST), ~5 undeclared (hoisting edge cases — composite members / forms scan_used$ misses), 4 `xb_inline` (INLINE$ maps to `xb_inline` but cgen.x's C prelude omits the helper Rust emits). Next: `xb_inline` helper + triage misc. | self-hosted demo AOT | infra |
