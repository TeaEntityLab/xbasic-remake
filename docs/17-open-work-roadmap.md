...
> `checks/validate-all.sh` passes **339 tests across 40 binaries**;
> `checks/verify-bootstrap.sh` is green, including `cgen_cemitter_sync`
> **85/85**. The LLVM feature gate passes **144 tests** with one documented
...
> kills when 85 `cgen_cemitter_sync` tests each spawn external cgen+cc
