...
| CGEN-SELFHOST-PARITY | self-hosted cgen.x | **The demo sweep/differential measure the Rust `CEmitter` (`--compile`), not cgen.x.** cgen.x's *true* demo faithfulness (build native cgen from cgen.x → IR→cgen→cc→run vs interp, 114 demos) was **faithful=1** — cgen.x was a **minimal self-hosting seed**. Now **faithful=109, diverge=0, cc-fail=0, crash=0, timeout=5** (2026-08-23: +CGEN-STRCMP-LEN fixing qbtoxb crash, +CGEN-MIXED-BYREF fixing arecurse crash, +CGEN-GUI-HEADLESS porting XuiGetNextCallback headless runtime + string SELECT CASE + integer byref fix, flipping 36 GUI demos from timeout to faithful, +CGEN-BYREF-WRITEBACK fixing adrawing cc-fail via all-byref scalar pointer params with copy-in/copy-out, +CGEN-SHARED-STR-INIT fixing amakemap crash via shared string init in main(), +CGEN-ARGSPLIT-STRLIT fixing string-literal-aware paren-depth counting, +RT-FLOAT-DIV/RT-ENTRY-PARAMS fixing DrawScaled/xgrids diverges via interp IEEE 754 float div + auto-vivify + parameterless entry, +RT-GOSUB-ZERO fixing agrids stack overflow via gosub_expr addr-0 skip). Sync 46/46, all suites green (1 ignored: `native_compiler_emits_cgen_ir_for_cgen` — compiler.x too slow for 5430-line cgen.x). | demo faithfulness | ✅ 109/114 |
