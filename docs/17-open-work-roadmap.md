...
| RT-COMPOSITE-ARR | interp + C backend | **Composite ARRAY member access is a broken subsystem** (root of the xin/xst/xit/xgr core-lib failures). A basic `DIM Rec r[N] : r[0].member = …` raises interp `unknown runtime slot r.member` (verified across empty/sized/module-level/type-prefixed decls); arecord "passes" only because BOTH backends mishandle it identically (its `type5[i].a` values never reach output — wrong-but-consistent → faithful). The flatten (`flatten_composite` → `self.arrays[var.member]`) never registers per-element runtime slots, and cgen never emits the member-array globals for a SHARED composite array (`SHARED FILE fileInfo[]` → `xb_str_fileInfo_fileName` undeclared, wrong `xb_str_` vs `xb_shared_` naming). **2-sided** (interp slot model + cgen emission), load-bearing, byte-identity-critical; needs full understanding of the partially-working arecord path first. Unblocks 4 core libs together. | composite arrays for real | subsystem (large) |
