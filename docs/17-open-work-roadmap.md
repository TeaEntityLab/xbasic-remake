...
| ~~RR-08a~~ | Behavior gates for pure libraries | **done 2026-08-30** — `pure_lib_behavior` test compiles xma.x + xcm.x + xut.x + xit.x via CEmitter and verifies 141 deterministic outputs: xma (42: 27 scalar math functions including COT via FPU wrapper mapping, at 0/1/2/0.5/PI inputs exercising full EXP/SQRT/LOG/ATAN/tan paths), xcm (91: all 22 DCOMPLEX + all 19 SCOMPLEX-returning functions + Atan2 edge cases + XcmVersion$), xut (1), xit (2), xst (5). CEmitter fixes: EXTERNAL builtin mapping (SQRT/SIN/COS→C helpers), x87 FPU wrapper mapping (XxxFPTAN→tan(), XxxFCOS→cos(), 30 wrappers total). Gates: sync 61/61, demo regression 27/27 | xdis |
