...
| CGEN-BYREF-ARG | C backend | scalar-vs-array by-ref args are ambiguous in the IR (`@a!` and `@a![]` both lower to `byref(symbol(a))`), and `ByRef(inner)⇒emit_expr(inner)` drops the `&` — so a scalar passed by-ref to an array param emits the value, not `&value` (int masked by `-Wno-int-conversion`, but **float `double`→`double *` is a hard cc error**). Needs by-ref to emit `&scalar` vs array-decay by-ref, disambiguated (needs a "is this name an array here" signal the FN context lacks for static 1-D arrays) + real write-back. **No corpus/demo impact** (corpus has 0 by-ref; no demo has float-by-ref since 114/114 compile) — surfaces only in XBSourceLib | XBSourceLib `ary`/`fgr`/`vgr`/merge (7 CFAIL) | feature |
