...
| CORE-LIBS-CC | C backend | `src/` system libs cc-clean: **✅ ALL 15 compile** (xcm, xdis, xma, xui, xut, xutpde, gdi32, kernel32, user32, xcol, xgr, xin, xit, xrun, xst). Two sessions (2026-08-23) landed 13 fixes (total errors 80→0): CGEN-SHARED-COMPOSITE, CGEN-TYPEAWARE-HOIST, CGEN-DUALUSE-SCALAR-PARAM, CGEN-SHARED-DIM-EXCLUDE, CGEN-AUTOSYM-DOLLAR, CGEN-STRPARAM-DUALUSE, CGEN-SHARED-ARR-NO-ARR, CGEN-OCTAL-LITERAL, **CGEN-EXIT-SELECT-TRUE** (EXIT SELECT → GOTO label in desugared SELECT CASE TRUE), **CGEN-STRUBOUND-HOIST** (hoist string scalars from UBOUND), **CGEN-CROSSFN-DUALUSE** (cross-function dual-use shared arrays), **CGEN-BYTE-ACCESS** (XBasic `{}` byte access on strings → `[l][n]`), **CGEN-DESC-SUFFIX** (`_d`→`_dd` avoids `#` collision), **CGEN-OWNNAME-INT** (return var for Integer fns using own name), **CGEN-DUP-LABELS** (disambiguate duplicate labels: SUB name vs explicit label — forward GOTO resolution), **CGEN-DESC-FORWARD** (declare pointer+ubound for undeclared @x[] into descriptor positions), **CGEN-NOT-STRING** (NOT on string = truthiness complement). All byte-neutral; every suite green. | all core libs | ✅ |
| CORE-LIBS-LINK | C backend | ✅ **done** (2026-08-23, `f8e34aa`): all 15 core libraries **link into one binary and execute** — the original XBasic build model (`src/Makefile` ar's xit/xst/xgr/... objects). Standalone per-lib compilation + link failed with 128 duplicate symbols: INTERNAL fns genuinely defined in several libs (`InitGui` in xit AND xst), Win32 shims (`GetLastError` in kernel32 AND xin), shared tables (`charsetSymbol`), `##`-system scalars. Fix: `XB_WEAK_SYMBOLS=1` env → `__attribute__((weak))` on `xb_user_` definitions and ALL file-scope globals (shared arrays, both `xb_shared_*` decl sites) + omit `main()`. Weak first-def-wins mirrors the emitter's own function dedup. Env unset by default → byte-identical; every suite green. Repro: `checks/link-core-libs.sh` → links 1690 `xb_user_` symbols + runs a cross-TU smoke (7 libs' `Version$` each return their exact source value — xcm 0.0007, the rest 6.4.5). | all core libs | ✅ |
