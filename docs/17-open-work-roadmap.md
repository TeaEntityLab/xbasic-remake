...
> — DCOMPLEX-returning functions now work: DCCONJ(3,4)=(3,-4) verified,
> composite return type emits as C struct (`xb_dcomplex xb_user_DCCONJ(...)`),
...
| ~~RR-08a~~ | Behavior gates for pure libraries | **done 2026-08-30** — `pure_lib_behavior` test compiles xma.x + xcm.x via CEmitter and verifies 31 deterministic outputs from compiled legacy bodies: xma (12: SINH/COSH/TANH/ACOS/ASIN/ATANH/LOG10/XmaVersion$), xcm (13: XcmVersion$/Atan2/DCABS/DCARG/DCNORM/DCCONJ), xut (1: XutInit), xst (5). DCCONJ tests composite return type: `FUNCTION DCOMPLEX DCCONJ(...)` emits as `xb_dcomplex xb_user_DCCONJ(...)` with struct return convention. CEmitter composite return support: struct typedefs (`xb_dcomplex`/`xb_scomplex`), composite return function signatures, struct local declaration, member assignment assembly at return, `.R` extraction in expression context, direct call emission in assignment context. Analyzer: `return_type_name` field propagated through AST/Parser/CheckedAST/IR; function name registered as composite var; `flatten_composite_return` decomposes `RETURN (ans)` into per-member assignments. Gates: sync 61/61, demo regression 27/27 | extend to DCSIN/DCCOS/DCEXP etc.; xdis (disassembler) |
