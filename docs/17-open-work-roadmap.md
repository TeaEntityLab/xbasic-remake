...
> `cgen_x_compiles_core_libs_floor_9_cc_clean`; 15/15 remains open**. All
> five failing libs (`xin`, `xit`, `xst`, `xcol`, `xgr`) are now
> `-Wint-conversion`/`-Wincompatible-pointer-types`-only (pre-existing in
> both cgen.x and Rust CEmitter; all real code errors fixed as of
...
> is parsed (AST `Statement::Attach`) but runtime semantics remain no-op;
> GUI is headless, and SHELL/network effects are
...
| now | **RR-03 scoped facets** | Implement scope-qualified facet lookup (scope+name+type) to advance selfhost core libs 9/15 → 13/15 |
...
| CGEN-LIB-SCALE | cgen.x | **9/15 at `54db874`** through both heuristic and narrow-facet probes. Passing: `xcm,xdis,xma,xut,xutpde,gdi32,kernel32,user32,xrun`. Classification/scope failures: `xui` (past `tool`/`window`, still `a`/`k`/`array` collisions), `xin`, `xit`, `xst`. Independent resource/signal failures: `xcol` (2.6 MiB IR, OOM/Killed:9) and `xgr` (abort/signal). | RR-03 closes classification to 13/15; RR-05 removes resource exits and closes 15/15. No whole-body substring masking. |
| ATTACH-IMPL | interpreter + C backends | **Parser implemented 2026-08-30** — `ATTACH` now parses into `Statement::Attach` (AST) → `CheckedItem::Nop` (lowers to `IrItem::Nop`). No longer skipped by parser. Runtime semantics still no-op (aliasing not implemented). Used in `xcol (122 sites), xst (61), xit (239), xgr (9)` — hash tables, file locks, charMaps, window kids, prog/func/tok arrays. Link succeeds but runtime array aliasing is silently wrong. Blocks tier (c) adopt for 5/15 libs. | implement ATTACH runtime semantics (pointer aliasing for dynamic arrays, memcpy for row-slice destinations) or explicitly accept semantic divergence + add deterministic ATTACH alias test |
...
| ARY-STATUS-RECONCILIATION | tests + docs | Two-stage status: RR-02 is compile-only composite-descriptor repair. Runtime ARY remains unready because `ATTACH` runtime semantics are not implemented (parser done, aliasing no-op) and no bounded behavior run is locked. | after RR-02, require RR-06 runtime semantics + bounded ARY behavior evidence before a runtime-faithful claim |
