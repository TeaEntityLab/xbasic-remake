...
> --exclude xb-ide` = **190 passed / 0 failed**; `cgen_cemitter_sync` **5/5**;
...
| RT-XST | interp + backends | real `XstStringToNumber`/`XstQuickSort`/`XstCopyArray` (all zero-stubs today). **🟢 UNBLOCKED (2026-08-21, `90dcfd4`): the GIANT-type prerequisite is LANDED.** The earlier "blocked on frozen GIANT model" claim was **disproven** — no v0.1 golden or bootstrap program uses GIANT-typed *variables* (`system_variables` uses the `$$`-PREFIX constant sigil; `bit_ops`/`bit_reinterp`/`type_conv_rotate` use GHIGH/GLOW/GMAKE/GIANT *builtins* on small values that render identically under i64). A real `RuntimeValue::Giant(i64)`/`ValueType::Giant` now exists across interp+cgen+cgen.x (arith/compare/coerce/render/builtins; GMAKE→64-bit, GHIGH/GLOW take Giant, GIANT() widens; large decimal literals type GIANT; c_type `int64_t`, xb_str/print/read_giant). So `value$$` can hold a 64-bit int or f64 bits. **Remaining = the `XstStringToNumber` parser** (spec `xst.dec:254` + `misc.hlp`; ref asm `xlib.s`). Contract (from `msc.x` wrappers): sig `(s$, startOff, @afterOff, @rtype, @value$$)`; returns specType=0 on success / -1 on format error; skip leading whitespace/unprintable from startOff; rtype∈{SLONG=6,XLONG=8,GIANT=12,SINGLE=13,DOUBLE=14}; `value$$` = integer value (SLONG/XLONG/GIANT) or f64/f32 bits (DOUBLE via DMAKE(GHIGH,GLOW) / SINGLE via SMAKE(GLOW)); afterOff = offset past the number. **Mechanism**: by-ref builtin (interp special-case in `call.rs` with write-back to args 2-4; cgen must emit `xb_xst_string_to_number(s, start, &after, &rtype, &value)` — a NEW by-ref-builtin emission path, distinct from `defined_param_byref`); parser written once in Rust + mirrored in `c_runtime.rs` + `cgen.x`; lock with an interp==cgen differential. **Atomic** — a partial landing regresses the faithful msc/fgr/vgr (both stub 0 today) | msc/fgr/vgr functional parsing | feature (unblocked) |
