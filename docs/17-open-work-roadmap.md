...
| ~~RR-08a~~ | Behavior gates for pure libraries | **done 2026-08-30** — `pure_lib_behavior` test compiles xma.x + xcm.x via CEmitter and verifies 85 deterministic outputs from compiled legacy bodies: xma (12: SINH/COSH/TANH/ACOS/ASIN/ATANH/LOG10/XmaVersion$), xcm (67: XcmVersion$/Atan2/DCABS/DCARG/DCNORM + all 22 DCOMPLEX-returning functions + 9 SCOMPLEX-returning functions: SCCONJ/SCSIN/SCCOS/SCCOSH/SCSINH/SCEXP/SCLOG/SCSQRT/SCRMUL), xut (1: XutInit), xst (5). DCOMPLEX and SCOMPLEX composite return types fully working including composite return → composite param chaining (DCPOWERCC/DCPOWERRC). Gates: sync 61/61, demo regression 27/27 | xdis (disassembler) behavior |
