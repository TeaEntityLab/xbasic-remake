...
| CGEN-LIB-SCALE | cgen.x | Heuristic `--emit-ir` probe at 8840f2a is **9/15** (was 6/15): `xcol` Killed:9 (OOM leaky concat), `xgr` Abort:6, `xui`/`xin`/`xit`/`xst` cc errors (string-array dual-use, host_address type, argv_s, OSTOXERROR); `xcm,xdis,xma,xut,xutpde,gdi32,kernel32,user32,xrun` pass. Facet probe (`emit_program_with_facets` → cgen) also 9/15 with same 6 fails (xst now `OSTOXERROR` vs missing file). `xgr` 1.41 MiB IR reached 6.4 GiB RSS. | fix nested-function depth handling and replace leaky whole-string concatenation (L11); then add a 15-lib cgen guard (see CGEN-X-LIB-COMPILE) |
...
| CGEN-X-LIB-COMPILE | self-hosted cgen.x | No named test emits the 15 core libraries through self-hosted `cgen.x` and compiles them. `cgen.x` demo compilation (114/114 at 1c2c929) does not cover lib-scale storage patterns (ATTACH-adjacent aliasing, composite member arrays, multi-dim shared arrays). Both heuristic and facet `--emit-ir` probes at 8840f2a are **9/15** (see CGEN-LIB-SCALE: xcol/xgr OOM, xui/xin/xit/xst cc errors). Blocked by CGEN-FACET-MANIFEST completion + L11 leaky concat. | land `cgen_x_compiles_all_core_libs_cc_clean` (build native cgen from `selfhost/cgen.x` via Rust CEmitter; emit-ir→cgen→`cc -O0 -w`; cc-clean per lib; falsified by SIGKILL/OOM/cc error/hang) |
