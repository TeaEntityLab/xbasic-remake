...
> --exclude xb-ide` = **191 passed / 0 failed**; `cgen_cemitter_sync` **5/5**;
...
| CGEN-REDIM / CGEN-BYREF-REDIM | C backend | `REDIM` emitted as `DIM` (fixed C arrays; no resize-preserving realloc). **REDIM-through-`@array[]` CRASHES cgen — verified 2026-08-21** (`FUNCTION Grow(@a[]) { REDIM a[4] }` called with `@n[]`: interp → `ubound=4, n3=99`; cgen → SIGABRT rc=134). **Concrete root cause (from emitted C):** `DIM n[1]` emits a *fixed stack array* `intptr_t xb_var_n_arr[2]` **plus** a separate scalar `intptr_t xb_var_n`; the call `@n[]` passes `&xb_var_n` (the **scalar**, wrong — not the array), and `UBOUND` reads `sizeof(xb_var_n_arr)/sizeof(..)-1` (ignores any resize). So by-ref arrays have no heap storage, no length cell, and the wrong pointer is passed. Needs the array ABI: heap-allocated arrays + a `{data, len/dims}` descriptor passed **by pointer** so a callee REDIM's realloc + new length propagate. **This is the blocker for RT-XST's `XstQuickSort` (14 uses — REDIMs `@n[]` to match `a[]`) and `XstCopyArray` (2 uses).** `XstStringToNumber` needed no arrays and is done; these two need this foundational unblock (the array analog of how GIANT unblocked the parser) | `XstQuickSort`/`XstCopyArray`; content-preserving REDIM | feature (foundational) |
...
| RT-XST | interp + backends | **`XstStringToNumber` ✅ done (2026-08-21, `6a5aa4e`)** — real number parser (skip whitespace; sign; hex/bin/oct; decimal int; float w/ exp) returning specType (0 ok / -1 error) + writing afterOff/rtype (SLONG6/XLONG8/GIANT12/DOUBLE14)/value$$ (int value or f64 bits). By-ref builtin: interp `call.rs` special-case with lvalue write-back; C backend `xb_xst_str_to_num(s,start,&after,&rtype,&value)` via a new by-ref-builtin emission (`emit_byref_value`/`emit_byref_addr`) + gated `emit_xst_runtime` (byte-for-byte port of `xst.rs`, byte-neutral on the Xst-free corpus → sync 5/5). Enabled by the GIANT type (`90dcfd4`). **msc/fgr/vgr now parse for real and stay byte-faithful** (interp==cgen); locked by `cgen_matches_interpreter_on_xst_string_to_number`. Suite 191/0, bootstrap intact, demos 74/0. **Remaining**: (a) `selfhost/cgen.x` doesn't yet mirror the Xst helper/emission — deferred (gated; no self-host tool uses Xst, so no-op on the bootstrap corpus); (b) `XstQuickSort` (57 uses) + `XstCopyArray` still zero-stubs (need the array ABI / by-ref array plumbing) | msc/fgr/vgr functional; `ary` `XstQuickSort` | feature (partly done) |
