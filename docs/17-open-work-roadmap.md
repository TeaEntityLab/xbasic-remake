...
> `checks/validate-all.sh` passes **350 tests across 41 binaries**;
> `checks/verify-bootstrap.sh` is green, including `cgen_cemitter_sync`
> **94/94**. The LLVM feature gate passes **144 tests** with one documented
...
> kills when 94 `cgen_cemitter_sync` tests each spawn external cgen+cc
