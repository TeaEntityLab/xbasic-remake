...
> test --workspace --exclude xb-ide` = **217 passed / 0 failed**; `cgen_cemitter_sync`
> **12/12** (byte-identity intact; +computed-GOTO +AT-write +pos/SIZE +giant-literal +float-arith +multidim +undeclared-local-hoist); +4 interp Giant-context fixes; +cgen.x float-arith, multi-dim array & undeclared-local hoisting fixes; native bootstrap fixed point **intact**; LLVM
...
| CGEN-SELFHOST-PARITY | self-hosted cgen.x | **The demo sweep/differential measure the Rust `CEmitter` (`--compile`), not cgen.x.** cgen.x's *true* demo faithfulness (build native cgen from cgen.x, then IR→cgen→cc→run vs interp over all 114 demos) was **faithful=1, cc-fail=112** — cgen.x is a **minimal self-hosting seed**, not a full generator, and only ever runs on the selfhost tools (subset: every local DIM'd, no unknown builtins, no shared arrays). Gaps by demo-blocker frequency: **(1) ~~undeclared-local hoisting~~ ✅ DONE (`d98c635`)** — cgen.x now mirrors `emit_hoisted_scalars` (buffer body, collect `symbol(n:t)`/`for`/`assign` scalars minus DIM/REDIM/params/retval, emit `T xb_var_n = default;` at prologue); byte-neutral on selfhost (0 undeclared locals there → sync 12/12, bootstrap intact), locked by `cemitter_and_cgen_agree_on_undeclared_local`. cgen.x differential **1→3 faithful**; ~85 demos now compile past the undeclared-local layer. **(2) UNKNOWN-BUILTIN decls — now the dominant wall (~61 Xui/Xgr GUI + ~24 Xst)**: a call to an undeclared builtin emits `xb_user_Xxx(...)` with no forward-decl → implicit-decl cc error (CEmitter forward-declares/stubs them; cgen.x must too). **(3) SHARED arrays** — CGEN-SHARED-ARR-SELFHOST. Also newly-visible once demos compile: FORMAT$/column formatting emits empty (aformat/acolumns), and a shared interp-vs-**both-C-backends** FOR-final-value gap (post-loop counter = limit+step in C, = limit in interp). Next: gap (2). | self-hosted demo AOT | infra |
