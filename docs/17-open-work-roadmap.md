...
| ~~RR-08a~~ | Behavior gates for pure libraries | **done 2026-08-30** — `pure_lib_behavior` test compiles xma.x + xcm.x via CEmitter and verifies 123 deterministic outputs from compiled legacy bodies: xma (30: SINH/COSH/TANH at 0 and 1, ACOS/ASIN/ATANH/LOG10/ACOSH/ACOT/ACOTH/ACSC/ACSCH/ASEC/ASECH/COTH/CSC/CSCH/SEC/SECH/LOG/XmaVersion$), xcm (87: XcmVersion$/Atan2/DCABS/DCARG/DCNORM + all 22 DCOMPLEX-returning functions + all 19 SCOMPLEX-returning functions), xut (1: XutInit), xst (5). CEmitter fix: EXTERNAL FUNCTION declarations for recognized builtins now map to C runtime helpers instead of zero-returning weak stubs. DCOMPLEX/SCOMPLEX composite return types fully working including composite return → composite param chaining (DCPOWERCC/DCPOWERRC/SCPOWERCC/SCPOWERRC). SCOMPLEX float precision handled via float-cast expected values. Gates: sync 61/61, demo regression 27/27 | xdis (disassembler) behavior; COT (depends on FPU stub XxxFPTAN) |
