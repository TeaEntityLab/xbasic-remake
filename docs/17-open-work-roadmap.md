...
| ~~RT-OPEN-SHARE-MODES~~ | interpreter + Rust C runtime + cgen.x C runtime | ✅ **done** (2026-08-24): OPEN decodes documented bases `0/1/2/3/4/0x10/0x20/0x30` after stripping orthogonal `0x800` NONBLOCK; unknown bases deterministically fall back to read-only existing-file behavior. Existing project semantics preserved: WR/WRNEW/RWNEW truncate+create; RW preserves+creates; WRSHARE/RWSHARE preserve+create; RDSHARE requires existing. Unix generated C uses `open(2)` + `fdopen` and applies `O_NONBLOCK`; Windows keeps a guarded `fopen` approximation (NONBLOCK accepted/no-op, share locking not claimed). Locked by a three-way filesystem matrix, invalid-mode coverage, interpreter `F_GETFL` assertion, and generated-C FIFO no-block test. LLVM remains a separate fd-runtime gap (its current `fopen` ABI cannot express exact WRSHARE/NONBLOCK semantics). Bootstrap+cgen suites 87/0. | shared/nonblocking OPEN | done |
