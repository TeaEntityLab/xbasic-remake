...
| adopt Rust-CEmitter library scaffolding | conditional | 15/15 compile/link plus 303 pure + 128 stateful behavior checks; no general compiled-body authority for SHARED-array-dependent functions |
| adopt self-hosted cgen for core libraries | compile-clean | 15/15 core libs compile via `emit_program_with_facets` + `-Wno-` flags (`8fe02ce`); 303+128 behavior checks via Rust CEmitter (not cgen.x) |
