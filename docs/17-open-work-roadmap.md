...
> --exclude xb-ide` = **188 passed / 0 failed**; `cgen_cemitter_sync` **5/5**;
> native bootstrap fixed point **intact**; LLVM **105/0**. C-backend demo sweep:
> **114/114 compile, 72 byte-faithful, diverge=2, 0 compile-fails** (up from
> 3→55→97→113→114). `acrc32` now byte-faithful after CGEN-SHIFT (i32 arithmetic
> via `(int32_t)` result masking). Beyond demos, **XBSourceLib core libs 13/13
> C-compile, 10 byte-faithful**
> (`msc`/`fgr`/`vgr`/`vgrOld`/`geo`/`mergeTest01`/`mergeTest02`/`mergeTest03`/`mergeOut`/`mergeOut02`;
> `geo` byte-identical after CGEN-BYREF-WRITEBACK + CGEN-FLOAT-FMT). The remaining
> divergers are RT-ARGS `XBMerge` and array-ABI `ary`/`ary1.0001`/`aarray`/`aarray_ISNODE`),
