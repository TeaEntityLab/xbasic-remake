...
| CGEN-BYREF-ARG | C backend | `ByRef(inner)⇒emit_expr(inner)` passes the value, so a by-ref arg to an *array/pointer* param (`double *`) gets a `double` — int is masked by `-Wno-int-conversion`, but **float `double`→`double *` is a hard cc error** (7 XBSourceLib libs). The fix is **callee-param-driven**, not a blanket `&` (verified 2026-08-20 — adding `&` to every scalar by-ref regressed msc: its `@value#` to a *scalar* `double` param then got `double *`, "remove &"). Correct rule: by-ref to an array/pointer param ⇒ `&scalar` (or the array, decayed); by-ref to a scalar param ⇒ value. That needs `emit_call_args` to carry each param's `is_array` and pick per arg — plus real write-back. **No corpus/demo impact** (corpus has 0 by-ref; no demo has float-by-ref since 114/114 compile) | XBSourceLib `ary`/`fgr`/`vgr`/merge (7 CFAIL) | feature |
