...
> test --workspace --exclude xb-ide` = **198 passed / 0 failed**; `cgen_cemitter_sync`
...
| CORE-LIBS-CC | C backend | `src/` system libs cc-clean: **7/18 compile** (CreateHelp/kernel32 + 5 others). 11 fail with distinct emit bugs, all parse+emit-C fine: **(a) undeclared `xb_shared_*`** — a `##`/composite-of-shared var used but not hoisted (MakeDistLinux/xutpde `XBSystem`, xui `STANDALONE`, xrun `XBDV`, xcol `UCODEZ`); **(b) undeclared `xb_str_*`** — composite/string member not declared (xit `dbase`, xst `fileInfo_fileName`, xgr `display_s`); **(c) undeclared `xb_var_*`** (xdis `imm`, xin `time`); **(d) builtin arg-count** (CreateHelp: 2 args to a 1-arg builtin). Libs are EXTERNAL-bottomed (not runnable) → bar is cc-clean, no differential. Foundational for *real* GUI-RUNTIME via IMPORT-linking (xgr/xui/xst are the demos' Xui*/Xgr* impls). | core libs migration | codegen (large) |
