...
> test --workspace --exclude xb-ide` = **202 passed / 0 failed**; `cgen_cemitter_sync`
...
| CGEN-SHARED-ARR | C backend | **SHARED arrays don't cross functions in cgen** (root of the xin/xst/xit/xgr core-lib failures — they use `SHARED … []`). *Correction*: composite ARRAY member access is NOT broken — it works with proper dotted members (`DIM Rec r[N] : r[0].member`, verified interp==cgen; my earlier "broken subsystem" repros were malformed TYPEs without the leading `.`). The real bug: a `SHARED a[]` (scalar-elem OR composite-member) is emitted as a *per-function local*, so each function gets its own uninitialized copy — verified runnable: `sa.x` interp `7/9` vs cgen garbage; `cF.x` interp `alice=7` vs cgen `=0`. The interp keeps them in `state.shared` (global). **Attempted fix (reverted 2026-08-21):** emit ONE heap global per shared array (reuse `xb_var_a`/`xb_ub_a` dyn machinery at file scope; skip local hoist + bare-`SHARED a[]` no-op; exclude from undimmed-fold). Fixed sa.x/cF.x + byte-neutral (sync 5/5, cgen.x uses no SHARED arrays) BUT regressed **dual-use** shared arrays: kernel32's `library.count` emits `xb_var_library_count_arr` (dual-use `_arr`) at access vs `xb_var_library_count` at the global decl (emit_globals lacks per-function dual-use context) → undeclared. Needs **program-wide dual-use detection** (or shared-aware `_arr` naming) to land. | 4 core libs | codegen (large) |
...
| CORE-LIBS-CC | C backend | `src/` system libs cc-clean: **11/18 compile** (was 6; +kernel32, +MakeDistLinux/xutpde, +CreateHelp, +xrun). 7 fail; each has a *tail* of distinct emit bugs (fixing the first reveals the next), so a lib flips only when all clear: **builtin arg-count** (xcol); **undeclared `xb_var_*`** hoisting/collision (xdis `imm`, xui `window`, xin); **undeclared `xb_str_*`** composite/string member — notably xst `SHARED FILE fileInfo[]` member array never emitted as a global + named `xb_str_` not `xb_shared_` (deeper SHARED-composite-array emission, unblocks xst/xit/xgr together), xgr `display_s` name-variant, xit `dbase`. Libs are EXTERNAL-bottomed (not runnable) → bar is cc-clean, no differential. Foundational for *real* GUI-RUNTIME via IMPORT-linking. | core libs migration | codegen (large) |
