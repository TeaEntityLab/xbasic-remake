...
| CGEN-XFUNC-SCALAR | C backend | fgr's chain (post by-ref + composite fixes) converges 5 libs on `xb_var_fillColour` **used in one function but declared only in a later one** — a cross-function scalar (`fillColour` Integer, distinct from `fillColour$` String) reached via `@fillColour` by-ref-to-scalar-param, whose write-back isn't modeled so the using function never declares it. Distinct from CGEN-BYREF-ARG (that was array params); this is a by-ref-target scalar needing per-function declaration (or the ground-up by-ref write-back model). No corpus/demo impact | XBSourceLib fgr-cluster (5) | feature |
