...
>
> **Current active regression ledger (2026-09-05):** all three 2026-09-01
...
>
> **Gate parallelism note (2026-09-05, cause measured 2026-09-06):** both
> `checks/validate-all.sh` and `checks/verify-bootstrap.sh` use
> `--test-threads=1` to prevent OOM process kills when 115 `cgen_cemitter_sync`
> tests each spawn external cgen+cc processes on an 8-core machine under load.
> `cargo test` runs test binaries sequentially; `--test-threads=1` only affects
> within-binary parallelism. The kills were not only concurrency: one native
> cgen.x on `xgr.x` peaked at 7.6 GB RSS (xit/xui larger, killed by signal)
> until CGEN-OOM below; single instances now peak at 1.8-3.6 GB, so two full
> suites side by side still exceed a 16 GB host - keep one at a time.
>
...
> is parsed (AST `Statement::Attach`) with move-semantics runtime (2026-09-05 reversal:
> src emptied, dst receives data/ub, runtime error if dst non-empty; undeclared
> destinations vivify as empty nodes; row ATTACH with no in-scope 2-D shape is
> a guarded no-op);
...
> Runtime behavior gates progressing: `ATTACH` move-semantics runtime is done
...
- The previous sole workspace failure (shared `ARY_VAR_DATA` descriptor) is now
  ...
  runtime `ARY` move-semantics now implemented (RR-06 done 2026-08-30; reversed to move 2026-09-05); bounded behavior test pending.
