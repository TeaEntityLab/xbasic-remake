...
| CGEN-DUALARR-DECL | C backend | fgr-cluster next blocker: the **array facet** `xb_var_px3D_shape_x_arr` of a dual-use name is undeclared when the name is an *undimmed* array (scalar `DIM px3D.shape.x` + `array_assign px3D.shape.x[k]`, no array `DIM`). The scalar facet is hoisted (emit_hoisted_scalars) but nothing declares the `_arr` facet (not dyn, not DIM'd as array). Needs the dual-use `_arr` facet declared for undimmed-array duals. No corpus/demo impact (no such dual there) | XBSourceLib fgr-cluster (5) | feature |
