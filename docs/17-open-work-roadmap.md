...
> test --workspace --exclude xb-ide` = **230 passed / 0 failed**; `cgen_cemitter_sync`
> **26/26** (byte-identity intact; +float-arith +multidim +hoist +entry +unknown-call +bare-return +dedup +arg-count +inline-helper +escaped-quote +type-suffix-sanitize +typed-unknown-default +leading-zero +EOF-arg +repeated-gosub +computed-goto-prologue +version-offbyone +AT-stub-0 +composite-dot-sanitize); +4 interp Giant-context fixes; cgen.x self-hosted-generator fixes lifting **self-hosted demo faithfulness 1→65** (true cgen.x differential, real bootstrap cc flags; the computed-GOTO-prologue fix replaced an earlier rewrite-to-return hack, matching Rust); native bootstrap fixed point **intact**; LLVM
...
| CGEN-SHARED-ARR-SELFHOST | self-hosted cgen.x | The CGEN-SHARED-ARR fix above landed in the **Rust CEmitter only** — it was **never mirrored into `selfhost/cgen.x`**. Rust `--compile` emits a shared array as a dyn heap global (`intptr_t* xb_var_g = 0;` + `calloc` at the DIM site + flattened `[i*(d1+1)+j]` access) and prints correctly; cgen.x instead sends the sized DIM `dim shared g:integer[3,3]` through its **local** `dim ` handler, which does not strip the `shared ` keyword (emits `xb_var_shared`, cc "redefinition") and has no heap-global path. So every `SHARED` **array** (as opposed to `##` shared scalar) compile-fails through cgen.x. Blocks CursorEdit + Kittedy (their `SHARED tbGridNum[]`/`squareInfo.grid[]`). UNCAUGHT: the selfhost tools use only shared *scalars* (`##`), never shared arrays, so the sync suite never exercises it. Fix = mirror Rust's `is_dyn_array`/`emit_dyn_decls` shared-global scheme into cgen.x (bootstrap-critical) — and its access **must be flattened** (`i*(d1+1)+j`), NOT the native `[i][j]` brackets CGEN-MULTIDIM emits for locals, since the store is a `T*` pointer. Repro: `SHARED g[] : DIM g[3,3] : g[1,2]=5 : PRINT g[1,2]` (interp + `--compile` = 5; cgen.x = cc fail) | ary-class self-hosted AOT | infra |
| CGEN-SELFHOST-PARITY | self-hosted cgen.x | **The demo sweep/differential measure the Rust `CEmitter` (`--compile`), not cgen.x.** cgen.x's *true* demo faithfulness (build native cgen from cgen.x → IR→cgen→cc→run vs interp, 114 demos) was **faithful=1** — cgen.x was a **minimal self-hosting seed**. Now **faithful=65, diverge=2 (atrim/awrite byte-level+file-IO), cc-fail=46** after 20 byte-neutral fixes (sync 26/26, bootstrap intact; +computed-GOTO-prologue replacing the dead-gosub-goto rewrite hack, +version-directive off-by-one, +*AT memory-read stub-0, +composite-member `.`→`_` sanitize — wviewbmp Now **faithful=63, diverge=2 (atrim/awrite byte-level+file-IO), cc-fail=48** after 16 byte-neutral fixes (sync 24/24, bootstrap intact) afuntype flipped faithful) — measured with the REAL bootstrap cc flags `-Wno-int-conversion -Wno-incompatible-pointer-types` (the earlier 47 used a stricter `cc -w`; the char↔ptr diagnostics below are warnings under `--compile`, not errors), each a Rust-CEmitter feature that was never mirrored: **(0) ~~entry point~~ ✅ `8484a72`** (call `Main` else first parameterless fn). **(1) ~~undeclared-local hoisting~~ ✅ `d98c635`**. **(2) ~~unknown-call drop~~ ✅ `b4e9413`** (external `Xui/Xgr/Xst` → no-op/`0`; was the ~85-demo wall). **(3) ~~dead gosub-goto~~ ✅ `187eecc`**. **(4) ~~function-dedup~~ ✅ `1025d87`** (first-wins; `INTERNAL FUNCTION`+`FUNCTION` same name → `redefinition`). **(5) ~~arg-count~~ ✅ `950a77c`** (`##funcArity$` table + `emit_args_n$` drop/pad, mirrors `emit_call_args`). **(6) ~~INLINE\$ helper~~ ✅ `0fb6402`** (emit `xb_inline` in the C prelude, gated on the IR containing `INLINE\$(` a real call so self-compile stays byte-identical; flipped hello/ahello/atask). **(7) ~~escaped-quote concat~~ ✅ `c34ca2b`** (`first_expr$` operand scanner now skips `\`-escaped chars in-quote; a `string("… \"")` no longer breaks the operator split — flipped tfont + 3 ttxtline + ~4 more, +8 faithful; resolved the former `symbol(`-literal-leak class). **(8) ~~type-suffix idents~~ ✅ `91b4235`** (`sanitize_ident$` maps `#!@&%`→`_d/_f/_a/_l/_h` in `c_var_name$` + the 3 `xb_shared_` sites, mirroring `sanitize_c_ident`; removes the invalid-identifier c_syntax class — correct+byte-neutral but the affected demos, e.g. adrawing, block further on nested functions). **(9) ~~typed unknown-call default~~ ✅ `c0691f5`** (a `$`-returning unknown external assigned to a string var drops to `xb_str("")` not `0`; fixed the sole diverge amerge). **(10) ~~leading-zero decimal literals~~ ✅ `fc4039a`** (`strip_zeros$`: `08`/`09` → valid, hex `0x..` left intact; atrim now compiles past the octal, blocks on a byte-level string diverge). Locked by 15 `cemitter_and_cgen_agree_on_*` tests. All key fixed demos verified still faithful (no regression). Also landed (11)~~EOF arg-drop~~ `c37946e`, (12)~~repeated gosub-return label uniquify~~ `d6aa05c` (cleared the label_redef cluster). **Remaining 48 cc-fails, by cluster: NESTED FUNCTIONS (19) + DUAL-DIM/dyn-names (11, a name DIM'd scalar-then-array needs Rust's dyn-pointer scheme) dominate; plus undeclared (5), shared-array-misparse c_syntax (5), composite-member (2), auto-array subscript (2), other (4). Every one is a complete-unit complex feature or multiply-blocked — the clean single-construct wins are exhausted.** **All clean single-construct wins are now exhausted; the remaining 54 cc-fails are dominated by nested functions (19) + SHARED arrays (18)** — two complex bootstrap-critical features that gate the rest. **Faithful is now gated: the remaining 68 cc-fails are MULTIPLY-blocked** (dedup+arg-count cleared layers on the same ~15 demos without flipping them — abuffer, e.g., now hits the next layer). Remaining blocker classes: **NESTED FUNCTIONS** (`INTERNAL FUNCTION` nested in a body — IR indents `function Callback`/`CreateWindow` inside `Circle`. These are NOT separable: they capture the parent's locals/params (`v0`,`r1`,`upperMessage`,`designX`…) and are invoked via computed GOSUB (`gosub_expr Sub[message]`) — i.e. closures/label-blocks sharing the parent scope. Rust emits NO `xb_user_Callback`; it inlines them as labeled blocks inside `xb_user_Circle`. cgen.x emits them as separate nested C function defs → "function definition is not allowed here". Mirroring Rust's inline-label-block scheme is the largest, most complex remaining feature); **NESTED FUNCTIONS (18)** + **SHARED arrays (17, CGEN-SHARED-ARR-SELFHOST)** now dominate. char↔ptr is NOT a cgen.x bug — an auto-declared array (`argv$[]` from `Xst*(@argv$[])`) is emitted as scalar `char*` by BOTH cgen.x and the Rust CEmitter, and `--compile` tolerates the `char↔ptr` diagnostic via `-Wno-int-conversion`; such demos compile+run faithfully under the real flags. hoisting edge cases (composites, ~3 undeclared) + assorted c-syntax/other (~16). The `symbol(`-literal-leak class is RESOLVED by the escaped-quote fix (7). A coordinated multi-fix push (nested-functions first) is needed to move faithful further. | self-hosted demo AOT | infra |
...
| CORE-LIBS-CC | C backend | `src/` system libs cc-clean: **12/18 compile** (was 6; +kernel32/MakeDistLinux/xutpde/CreateHelp/xrun/xdis this session). The 6 fails (xui/xit/xin/xcol/xst/xgr) each bottom out on the **hardest dual-use / composite codegen** (each has a tail; a lib flips only when all clear; EXTERNAL-bottomed → cc-clean bar, no differential): **xin** — `SHARED SOCKET socket[]` is a dual-use SHARED composite array AND a function name (`socket()`) AND a scalar; array facet not `_arr`-tagged (`socket[socket]`) + `host.address` pointer-vs-scalar. **xst/xit/xgr/xui/xcol** — 20+ tails of undeclared composite/`_s`-variant/dual-use names + `subscripted value is not an array`. All in the delicate dual-use/composite path; each is a careful per-lib pass. Foundational for *real* GUI-RUNTIME via IMPORT-linking. | core libs migration | codegen (large, per-lib) |
...
| CGEN-EXPR-GAPS | self-hosted cgen.x | A **completeness audit** (does cgen.x `emit_expr$` have an arm for every `IrExprKind` the Rust CEmitter emits?) found exactly 2 unexercised gaps — both real Rust-vs-cgen.x drifts: **`funcaddr(` (`&Func()`, `FuncAddr`)** — cgen.x drops it (repro `p = &Helper() : IF p <> 0` → interp/rust print, cgen.x skips); needs the program-order function-id map (Rust `func_addr_id`/`FUNC_IDS`), which cgen.x lacks. **`byref(` (`@x` by-ref arg, `ByRef`)** — cgen.x passes by value, losing the write-back (repro `Bump(@x)`: interp/rust `6`, cgen.x `5`); needs cgen.x-side CGEN-BYREF (pointer param decls + `&arg` + callee by-ref flags). Statement dispatch is **complete** (every emitted token has an arm; LEFT$/MID$ offsets audited clean). Both gaps need global-state infrastructure `emit_expr$(e$)` lacks → fresh runway, not a session-tail edit to the bootstrap-critical generator | full cgen.x IR-construct parity | infra |
...
| CGEN-TYPED-ARRAY | self-hosted cgen.x | cgen.x emits **invalid C for typed arrays** whose slot-name keeps a type suffix. `DIM a#[3] : PRINT a#[0]` → cgen.x `intptr_t xb_var_a#[…]` (wrong element type — `intptr_t` not `double` — AND `xb_var_a#` is not a legal C identifier → cc fails); Rust emits `double xb_var_a_d[…]`. Array slot-names intentionally keep the suffix (`parser_cursor.rs:402`, so a callee `a#[]` binds), while scalar names strip it; Rust sanitizes every name via `sanitize_c_ident` (`#`→`_d`, `$`→`_s`, `!`→`_f`, `@`→`_a`, `&`→`_l`, `%`→`_h`, `.`→`_`), cgen.x has none. Fix: sanitize the suffix chars in cgen.x array names WITHOUT touching `.` (cgen.x emits composites as C **structs** — `xb_var_t.a` is valid member access, not a name to flatten) + use `c_type$(varType$)` for the array element type (non-string Dim path ~line 2140). Affects all typed arrays (float/single/byte/typed). UNCAUGHT: 0 typed arrays in the v0.1 corpus + selfhost tools (bootstrap unaffected); demos use the rust-direct path. Repro: `DIM a#[3] : a#[0]=1.5 : PRINT a#[0]` **BROADER than arrays:** the same unsanitized suffix also hits SCALAR/SHARED vars — adrawing emits `intptr_t xb_shared_RandomNSeed# = 0;` (a `#` double-suffix on a SHARED scalar → cc "expected ';'"), part of the c_syntax cc-fail cluster. A full fix must sanitize `#!@&%`→`_d/_f/_a/_l/_h` in BOTH `c_var_name$` AND the shared-name path (whose `#`/`##`-prefix `fwdHash` stripping is fragile — needs care to not break `##`-shared handling). | typed-array cc-validity | infra |
