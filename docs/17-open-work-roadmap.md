...
| ~~RR-08a~~ | Behavior gates for pure libraries | **done 2026-08-30** — `pure_lib_behavior` test compiles xma.x + xcm.x via CEmitter and verifies 69 deterministic outputs from compiled legacy bodies: xma (12: SINH/COSH/TANH/ACOS/ASIN/ATANH/LOG10/XmaVersion$), xcm (51: XcmVersion$/Atan2/DCABS/DCARG/DCNORM/DCCONJ/DCSIN/DCCOS/DCCOSH/DCSINH/DCEXP/DCLOG/DCSQRT/DCRMUL/DCPOLAR/DCLOG10/DCTAN/DCTANH/DCPOWERCR/DCACOS/DCASIN/DCATAN/SCCONJ/SCSIN/SCCOS), xut (1: XutInit), xst (5). DCOMPLEX and SCOMPLEX composite return types fully working: `FUNCTION DCOMPLEX DCCONJ(...)` emits as `xb_dcomplex xb_user_DCCONJ(...)` with struct return convention. SCOMPLEX returns `xb_scomplex` (float members). CEmitter composite return support: struct typedefs (`xb_dcomplex`/`xb_scomplex`), composite return function signatures, struct local declaration, member assignment assembly at return, `.R` extraction in expression context, direct call emission in assignment context. Analyzer: `return_type_name` field propagated through AST/Parser/CheckedAST/IR; function name registered as composite var; `flatten_composite_return` decomposes `RETURN (ans)` into per-member assignments. Gates: sync 61/61, demo regression 27/27 | extend to DCPOWERCC/DCPOWERRC; xdis (disassembler) |
