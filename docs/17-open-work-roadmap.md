...
| CORE-LIBS-CC | C backend | `src/` system libs cc-clean: **13/18 compile** (was 12; +xin this session). Four fixes landed (2026-08-23): **CGEN-SHARED-COMPOSITE** — shared composite array members (dotted names like `host.address`) now get global declarations despite being scalar-DIM'd elsewhere; only array DIMs skip the dyn_array reset path; scalar DIM symbols noted in `walk_items` for dual-use detection. **CGEN-TYPEAWARE-HOIST** — params and dimmed sets keyed by `(name, is_string)` so a string param `addr$` doesn't suppress the integer local `addr` (different C variables). **CGEN-DUALUSE-SCALAR-PARAM** — non-string scalar params used as arrays (`GOSUB @mode[mode]`) get dual-use split: scalar facet stays as param, array facet (`_arr`) declared as dyn array. **CGEN-SHARED-DIM-EXCLUDE** — shared DIMs excluded from scalar noting to prevent false dual-use detection. **xin** 9→0 errors. **xit** 20→7. **xst** 20→17. **xgr** 20→10. Remaining 54 errors: xcol=20 (string array `_arr` naming), xit=7 (label/function-name issues), xst=17 (built-in shared vars ARGV\$/OSERROR\$), xgr=10 (dual-use shared vars like borderWidth). Byte-neutral: sync 46/46, demo regression 27/27. | 1 core lib (xin) | ⏳ |
