...
lever left**. The 5 interpreter-clean programs that still diverge, by root cause
(byte-faithful PRINT output, `MID$`/`s${n}=v` byte-assignment, the auto-vivified-scalar
prealloc, `$$` constant evaluation, the `OPEN`/`CLOSE`/`LOF` file runtime, and the
`WRITE`/`READ [n]` record data path were resolved this session; see the summary):
...
**100/105 faithful, 0 compile-fails** (byte-strings resolved RT-BYTESTRING; byte-faithful
...
unblocked `aarray_ISNODE`). The residual 5 are the large/fundamental efforts tabled above.
