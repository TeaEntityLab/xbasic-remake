...
| ~~LEGACY-LIB-BINDING-POLICY~~ | frontend + C backend + runtime | ✅ **done 2026-08-30** — 299 behavior checks prove user-defined functions take precedence: `pure_lib_behavior` (175) + `stateful_lib_behavior\` (96) link compiled `xb_user_*` weak exports with C harness and verify deterministic outputs from user-defined bodies (xma SINH/COSH/ACOS, xcm DCCONJ/DCSQRT, xst XstGetOSName/XstVersion$, etc.). CEmitter `is_defined_func` gate + interp `find_function` ensure user-defined precedence over native helpers. | — |
| ~~CORE-LIB-BEHAVIOR-GATE~~ | tests | ✅ **done 2026-08-30** — 299 behavior checks across `pure_lib_behavior` (175: xma 56, xcm 107, xut 1, xit 8, xdis 3) + `stateful_lib_behavior\` (86: xst getters, setters, string functions, exception mapping, byval RETURN paths). Non-Version$ functions behavior-locked via C harness linking compiled `xb_user_*` exports. | — |
...
| ~~L3~~ | Non-Version$ core-library behavior test | **done 2026-08-30** | 299 behavior checks: `pure_lib_behavior` (175: xma/xcm/xut/xit/xdis) + `stateful_lib_behavior\` (86: xst) | — |
