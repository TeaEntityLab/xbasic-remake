...
| CGEN-SHIFT | C backend | cgen values are `intptr_t` (i64); the interp is **i32** (`RuntimeValue::Integer(i32)`, `wrapping_shr` = arithmetic i32 shift). acrc32's `table[1]=0x09073096` (vs standard unsigned `0x77073096`) is the interp's signed-i32 arithmetic-shift result. **Two experiments 2026-08-21, both reverted.** `c_type(Integer)="int32_t"` + `-fwrapv` makes acrc32 faithful via the CLI and is byte-neutral on the corpus (`cgen_positive_corpus` passes, 72/114 demos faithful). BUT it is **not shippable as a blanket change**: (a) signed overflow is UB → needs `-fwrapv` at every cc site (CLI + 7 test files + 2 bootstrap scripts; centralize via a `common::cc::cmd()` helper); (b) **DEFINITIVE BLOCKER** — `c_type(Integer)` is used for BOTH integer *values* AND integers that hold *code addresses*: computed `GOSUB`/`GOTO` store `(intptr_t)&&label` (a real 64-bit pointer) in an integer, and int32_t **truncates it → jump to garbage → crash**. Confirmed by `computed_gosub_test` faulting under the sync test (ASLR-flaky: `cgen_positive_corpus` passed by luck, `cgen_cemitter_sync` crashed). The correct fix needs **type-flow analysis** to emit int32_t for value-integers while keeping label/func-address integers `intptr_t` (or a tagged `IrExprKind` for address-valued ints), plus the cgen.x mirror. Deferred as a coordinated feature | `acrc32` (diverge) | feature |
