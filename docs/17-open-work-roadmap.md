...
> Historical full re-verification snapshot: **2026-08-31** — the full narrative
> (C runtime + interpreter hardening audit, twenty-six CEmitter fix batches,
> XBSourceLib/demo/LLVM counts of that date) is archived verbatim in
> docs/22 §1; the active ledger above supersedes its counts.
...
* **Inventory:** 15 libraries in `xbasic/lib/` (upstream `src/shared` 6 + `src/linux` 9, ported verbatim by TRACKED-CORPUS-PORT 2026-08-31). The harness performs
...
The execution queue and open-rows table below are the "what's left" view.
Rows marked done / ~~struck~~ stay as closed records with their exit gates and
evidence; unstruck rows are the open work. Older narratives are archived in
docs/22.
...
The 2026-08-17..20 narratives that occupied these sections are in docs/22 §2.
Everything in them is closed, superseded, or re-rowed above; nothing there is
an open row.
