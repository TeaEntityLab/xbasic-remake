...
| ~~RR-08a~~ | Behavior gates for pure libraries | **done 2026-08-30** — `pure_lib_behavior` test compiles xma.x + xcm.x + xut.x + xit.x via CEmitter and verifies 135 deterministic outputs: xma (40: 26 scalar math functions at 0/1/2/0.5 inputs exercising full EXP/SQRT/LOG/ATAN paths), xcm (87: all 22 DCOMPLEX + all 19 SCOMPLEX-returning functions), xut (1), xit (2), xst (5). CEmitter fix: EXTERNAL FUNCTION declarations for recognized builtins now map to C runtime helpers. Gates: sync 61/61, demo regression 27/27 | xdis; COT (FPU stub) |
