...
> test --workspace --exclude xb-ide` = **201 passed / 0 failed**; `cgen_cemitter_sync`
...
| CORE-LIBS-CC | C backend | `src/` system libs cc-clean: **10/18 compile** (was 6; +kernel32, +MakeDistLinux/xutpde, +CreateHelp). 8 fail; each has a *tail* of distinct emit bugs (fixing the first reveals the next), so a lib flips only when all are cleared: **builtin arg-count** (xcol); **undeclared `xb_var_*`** hoisting/collision (xdis `imm`, xui `window`; xin's `time` fixed via CGEN-SIZE-SUFFIX, 8 more remain); **undeclared `xb_str_*`** composite/string member — notably xst `SHARED FILE fileInfo[]` member array never emitted as a global + named `xb_str_` not `xb_shared_` (deeper SHARED-composite-array emission), xgr `display_s` name-variant, xit `dbase`; **unary type** (xrun: `char*` to a unary op). Libs are EXTERNAL-bottomed (not runnable) → bar is cc-clean, no differential. Foundational for *real* GUI-RUNTIME via IMPORT-linking (xgr/xui/xst are the demos' Xui*/Xgr* impls). | core libs migration | codegen (large) |
