...
> = **277 passed / 0 failed across 33 binaries** — **carried from prior banner** (not re-run this session; this session only ran `cargo test --release -q 212s exit 0` which is the truncation anti-pattern `validate-all.sh` was written to prevent — prior local full run was 274/0). `cgen_cemitter_sync` = **60/60**
> (now asserting corpus emitted-C byte equality + all-demo cgen.x compilation — 60/60 is the named lock; filtered `1 passed in 67s` ≠ full suite); positive-corpus emitted C is **80/80 byte-identical** between the Rust CEmitter
> and self-hosted `cgen.x`; all **114/114 demos** compile through true `cgen.x` (`cgen_x_compiles_all_demos_cc_clean`).
> That cgen.x all-demo guard is **compile-only** (Rust CEmitter differential **112 comparable / 2 I/O skips / 0 failures** is the behavior lock). Stage 0, Stage 1, and Stage 2 native self-hosting are complete; all 15 core libraries compile and link through the Rust CEmitter — **this session `link-core-libs.sh` at `3c5d9a7` + uncommitted tree → 1690 `xb_user_*` (Darwin `nm -U` `--defined-only`; GNU `nm -U` is unicode; count via `grep -c _xb_user_`); banner still carries **1736 after `14f9c69`** (xma EXTERNAL/nesting/NaN) — treat 1736 as **carried**, 1690 as **observed this session**; neither is body coverage**. `XB_WEAK_SYMBOLS=1` emits `__attribute__((weak))`, omits `main`, masks duplicates via **first-definition-wins by link `$OUT/*.o` glob order** (not named `cc` order; `OUT` never cleaned — leftover `.o` can change link/nm; `CC` is honored, `XB_BIN` is computed and wins over env). Wall 8.28s on this machine. `multi_lib_integration` executes only seven cross-TU `Version$` calls; native helpers (`XstQuickSort`/`XstCopyArray`/`XuiGetNextCallback`/kernel32 `GetStdHandle`…) still **shadow** compiled `xb_user_*`; it does not behavior-lock legacy library bodies. Historical session notes below preserve their original measurements and may be superseded by this banner or later dated rows.
...
> `ary`/`ary1.0001` are only cc-clean in `xbsourcelib_ary_compiles_clean`; they
> are not executed by the parity guard. Their remaining runtime status is
> **contested**: that test documents a crash/composite-byref gap, while later
> roadmap rows identify `ATTACH` aliasing and O(n²) interpreter lookup. Until a
> named run or timeout test resolves those accounts, do not claim they migrate
> faithfully or that interpreter performance is their only blocker.
...
| LICENSE-BOUNDARY | distribution + legal | 15 libs are a GPL/LGPL/unspecified mix: `xcol`/`xit`/`xdis` GPL (`COPYING`); `xst`/`xma`/`xcm`/`xui`/`xgr`/`xin`/`xrun`/`xut`/`xutpde` LGPL (`COPYING_LIB` truncated at §0, no GNU title/preamble); `gdi32`/`kernel32`/`user32` no notice; remake crates split `GPL-2.0-or-later` vs `LGPL-2.1-or-later` — `link-core-libs.sh` links GPL+LGPL+unspecified into one `xblibs` binary; distribution without GPL notices is not allowed | add § license table per L15; mark "port" claims as GPL-derivative; local smoke is run-only; fix `src/shared` (6: `xcm xdis xma xui xut xutpde`) vs `src/linux` (9) inventory |
...
| L11 | `xb_append` cap + `collect_append_chain` (uncommitted 2026-08-27) | **deferred — not adopted** | `c_runtime.rs` `xb_append` with one-shot `cap` (header stores length, not capacity; every call reallocs; NULL `(size_t*)a-1` is UB on `=0` shared strings; leaks on realloc failure; aliasing/moved-realloc hazard) + `c_emit_stmt.rs` left-assoc rewrite (`x$=x$` empty-chain panic, alias-unsafe) — mirrored in `selfhost/cgen.x` header only (generated user C still `xb_concat`) | adopt only with cap/header invariant test + alias guard + empty-chain guard + `cgen.x` assignment emission using `xb_append`; else revert before throughput claims |
| L12 | `scan_dyn` per-line `symbol(` walk (uncommitted 2026-08-27) | **deferred — not adopted** | DIM/SWAP already per-line; new code converts `symbol(` + `byref(` walk from whole-`s$` `INSTR(MID$)` to per-line `INSTR(ln$,nSym$)` with inner `WHILE` for multi-`symbol(` | add fixture with `string("symbol(foo:bar)")` + multi-`symbol(` on one line; prove no quote-skip regression; cross-line colon now dropped (fix, not byte-identity) |
| L13 | `scan_undimmed` MID-avoidance 3-arg `INSTR` (uncommitted 2026-08-27) | **deferred — not adopted** | `INSTR(MID$(s$,p+13,…),":")` → `INSTR(s$,":",p+13)` + `MID$(s$,p+13,e-p-13)` — algebra-equivalent first-colon; still whole-`s$` (not per-line) | name fixture with `array_access(` line containing `string("a:b")` inside literal; verify no false-colon |
| L14 | `pNames$[32]→[128]` + zap `argv$` nested-DIM facet | **deferred** | `xbasic-6.4.5/demo/zap.x` `DIM argv$[3]` is inside nested `IFZ standalone` THEN+ELSE (DIM twice) → `DynWalk` nested + `dim_count==2` ⇒ `storage=dyn` not `fixed`; docs/19 previously said `storage=fixed` (wrong) | dump `facet argv$:string scope=Entry storage=dyn rank1` + member-2D hardcode fix (`rank=2`/`shared` no-op) before claiming lib facet coverage |
...
> **Required wording changes applied to this roadmap:** banner provenance split (1690 observed vs 1736 carried + `validate-all.sh` not re-run this session), `XB_WEAK_SYMBOLS`/`CC`/`XB_BIN`/`OUT`/`nm` guardrails, 15-lib inventory (`shared` 6 vs `linux` 9) + **license-boundary blocker** (L15: GPL `xcol`/`xit`/`xdis` vs LGPL 9 vs 3 unspecified Win32 shims; `COPYING*` truncated; combined link/distribution is GPL-covered; local smoke is run-only), Bar A/B/C wording (Bar A = compile/link not behavior; Bar B = named `cgen_x_compiles_all_core_libs_cc_clean` does not exist), facet wording (slice 8 demo-scoped, `strDual`/`allStrArr` reverted to heuristic per slice 4.1), `xb_append`/`scan_dyn`/`scan_undimmed`/`pNames` L11–L14 deferred. Full per-lens markdown preserved in `agent://` (ProvenanceSecurityReviewer, StrategicSynthesisReviewer, CorrectnessReviewer-2, ReproducibilityEngineer).
> **L11–L17 added above** capture the uncommitted candidates as deferred ledger rows per the falsifiability protocol (§4 of `reflective-prompt-library/04-agent/workflow-recipes.md`). Partial adoption without these IDs is the drift this review exists to catch.
