...
> the 4 divergers are by-ref-write-back `geo` (`GeoNormal`'s `@nx!` writes aren't
> copied back → `0 0 0 0`; FLOAT-FMT latent behind it), RT-ARGS `XBMerge`, array-ABI `ary`/`ary1.0001`),
...
| CGEN-BYREF-WRITEBACK | C backend | scalar `@`-param **write-back** isn't modeled (cgen scalar params are by-value), so a callee that writes a result through `@nx!` etc. never reaches the caller — `geo`'s `GeoNormal(...,@nx!,@ny!,@nz!)` → caller reads `0 0 0 0`. **Design (turnkey)**: the interp models by-ref as pass-by-value + copy-back after the call (`call.rs` `writebacks`); C maps this to **copy-in/copy-out** — thread `by_ref` (already on AST `Param`, dropped at `CheckedParam`/`IrParam`) to `IrParam`; emit a by-ref scalar param as `T* x_ref`, prologue `T x = *x_ref;`, epilogue `*x_ref = x;` before every return, body unchanged; call sites pass `&arg`. Complication: epilogue before *each* return (multi-exit) — likely a single goto-epilogue. Byte-neutral on corpus (0 by-ref); demo-gated. FLOAT-FMT (`geo`'s `39.99990409954015` shortest-round-trip) is latent behind it | `geo`, aids others | feature |
