...
| ~~MODULE-DIM-SCOPE~~ | C backend | ✅ **done** (2026-08-24): top-level fixed-size non-string non-shared non-dyn array DIMs hoist to file-scope `static` declarations (`emit_module_dims`) when the program defines any function; hoisted names register as "dimmed" in every function context (`HOISTED_MODULE_DIMS`) so accesses don't fold to undimmed defaults; filtered from main()'s body. Byte-neutral behaviorally; sync 48/48, suite 259/0. Verified: module DIM + named Main + SWAP now matches interp (was 0 0, now 0 100). cgen.x keeps main-local emission (behavioral parity holds via its own scoping) — coordinate later if C-byte parity matters.| Programs with module-level DIMs + named functions | partial |
