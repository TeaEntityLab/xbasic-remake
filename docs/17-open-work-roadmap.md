...
| ~~LEGACY-LIB-BINDING-POLICY~~ | frontend + C backend + runtime | ✅ **done 2026-08-30** — 275 behavior checks prove user-defined functions take precedence: `pure_lib_behavior` (175) + `stateful_lib_behavior\` (96) link compiled `xb_user_*` weak exports with C harness and verify deterministic outputs from user-defined bodies (xma SINH/COSH/ACOS, xcm DCCONJ/DCSQRT, xst XstGetOSName/XstVersion$, etc.). CEmitter `is_defined_func` gate + interp `find_function` ensure user-defined precedence over native helpers. | — |
| ~~CORE-LIB-BEHAVIOR-GATE~~ | tests | ✅ **done 2026-08-30** — 275 behavior checks across `pure_lib_behavior` (175: xma 56, xcm 107, xut 1, xit 8, xdis 3) + `stateful_lib_behavior\` (86: xst getters, setters, string functions, exception mapping, byval RETURN paths). Non-Version$ functions behavior-locked via C harness linking compiled `xb_user_*` exports. | — |
...
| ID | Candidate | Status | Evidence | Next action / trigger |
|---|---|---|---|---|
| ~~L1~~ | Separate compile/link/behavior/production readiness wording | **done 2026-08-30** | 15/15 Rust compile/link; 275 behavior checks (pure 175 + stateful 100); 15/15 cgen.x compile floor. Readiness classes: (a) emit+cc-clean ✓, (b) multi-lib link ✓, (c) behavior of compiled legacy body ✓ (275 checks), (d) production runtime — open (GUI-RUNTIME, packaging). | — |
| ~~L2~~ | Native-vs-legacy call-binding policy before behavior-port claims | **done 2026-08-30** | `pure_lib_behavior` + `stateful_lib_behavior` tests link compiled `xb_user_*` exports (weak) with C harness; 275 checks verify user-defined bodies execute, not native helpers. `is_defined_func` gate in CEmitter + `find_function` in interp ensure user-defined precedence. | — |
| ~~L3~~ | Non-Version$ core-library behavior test | **done 2026-08-30** | 275 behavior checks: `pure_lib_behavior` (175: xma/xcm/xut/xit/xdis) + `stateful_lib_behavior\` (86: xst) | — |
...
| ~~RR-08b~~ | Behavior gates for stateful libraries | **done 2026-08-30** — `stateful_lib_behavior` test compiles xst.x via CEmitter and verifies 100 deterministic outputs: XstGetOSName(@name$)="unix" (SELECT CASE: ##XBSystem=0 != $$XBSysLinux=1, now correctly resolved from xut.dec), XstGetConsoleGrid(@grid)=0, XstVersion$()="6.4.5", XstGetEndianName(ret)=0, XstGetCPUName(ret)=0, XstGetApplicationEnvironment(@standalone,@reserved)=(0,0,0), XstExceptionToSystemException(1,@sig)=11 (SegViolation->SIGSEGV), (4,@sig)=2 (BreakKey->SIGINT), (7,@sig)=8 (DivByZero->SIGFPE), (12,@sig)=4 (InvalidInstr->SIGILL), (14,@sig)=11 (StackOverflow->SIGSEGV), (99,@sig)=11 (CASE ELSE->SIGSEGV). Parser fix (725a833): `is_forward` detection for `END EXPORT` and `EXTERNAL ##` preve…
...
> frontend 24/24, compiler 68/68, behavior 275 checks (175 pure + 100 stateful), demo 27/27, sync 63/63.
