...
| **M1 architecture — in progress** | **CGEN-FACET-RETIREMENT — `allStrArr` facet-driven consumption landed; scanner kept as headerless fallback. Cannot delete `scan_all_strarr$` until compiler.x emits facets (bootstrap IR_IDENTICAL). `xstArrays` invariant refined 2026-09-04: classified-storage (dyn/shared/param) 0/0 — the 4 former not-facet-dyn names are shared globals / array params. Next: `strDual` (use-based vs DIM-based dual is a semantic decision, not a mechanical cutover).** | 234-program ratchet: `allStrArr` 0/0; `strDual` 0/187; `xstArrays` unclassified 0/0. Headerless producer (compiler.x `--emit-ir`) is the cutover blocker. Verified: 316/316 gates target, 71/71 sync, 15/15 libs, 114/114 demos. |
| **M1 runtime — in progress** | **ATTACH-MOVE-SEMANTICS — reversal to legacy move semantics (2026-09-04)** — `ATTACH src TO dst` moves array (src becomes empty, dst receives data/ub, runtime error if dst non-empty) per `xbasic/helpsrc/help_text/lang.txt:57-62` and `L2244-2260` (`DIM d[3,]` + `ATTACH x[] TO d[0]` = independent jagged rows with distinct bounds `11, 15, 31, -1`). Reverses the 2026-09-04 alias/copy attempt (`c_emit_attach.rs` reversed direction; interpreter rectangular stride-splicing). Under move semantics, `ary.x` row-growth (`ATTACH row TO a[]` → `REDIM a[...]` → `ATTACH a[] TO row`) is simple move-out/1-D REDIM/move-back. | Replace view/copy paths across interpreter and C backends with jagged row cells + move semantics; re-derive test locks from lang.txt contracts. |
