...
| CGEN-X-LIB-COMPILE | self-hosted cgen.x | No named test emits the 15 core libraries through self-hosted `cgen.x` and compiles them. `cgen.x` demo compilation (114/114 at 1c2c929) does not cover lib-scale storage patterns (ATTACH-adjacent aliasing, composite member arrays, multi-dim shared arrays). Heuristic `--emit-ir` probe is 6/15 (see CGEN-LIB-SCALE). Blocked by CGEN-FACET-MANIFEST completion. | land `cgen_x_compiles_all_core_libs_cc_clean` (build native cgen from `selfhost/cgen.x` via Rust CEmitter; emit-ir→cgen→`cc -O0 -w`; cc-clean per lib; falsified by SIGKILL/OOM/cc error) |
