...
| CGEN-SELFHOST-PARITY | self-hosted cgen.x | **The demo sweep/differential measure the Rust `CEmitter` (`--compile`), not cgen.x.** cgen.x's *true* demo faithfulness (build native cgen from cgen.x → IR→cgen→cc→run vs interp, 114 demos) was **faithful=1** — cgen.x was a **minimal self-hosting seed**. Now **faithful=95, diverge=3 (aback/atrim byte-level + acrc32 interp-CRC-bug, not cgen.x faults), cc-fail=15** (2026-08-22: +nested-fn, +1-D dyn-array family, +scalar-DIM dedup, +undimmed/UBOUND folds, +per-function cross-function scoping `8a6e99d`, +forward-ref-scalar-hoist & C-name type-collision-dedup `f7bc9ec` flipping xgrids, +scalar-string-DIM-of-dyn-string-array skip flipping aprofile+adata, +SHARED-array heap-globals & undeclared-shared-scalar decl flipping amemory+amakemap, +`&func` decl-order id via `##funcIds$` flipping atimer, +2-D fixed+dyn arrays: fixed `string` via `emit_msub$`/`emit_mtotal$` flat-cast init, dyn via flattened calloc + row-major `g[i*(xb_d1+1)+j]` access (`scan_arr2d$`/`emit_flat2d$`/`emit_d1$`, `xb_d1_X` capture) — 0-flip alone but advances the 2-D cluster, e.g. aarray_ISNODE 4→1 errs; the 2-D demos also need by-ref-array-params + case-B)
