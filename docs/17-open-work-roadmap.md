...
> test --workspace --exclude xb-ide` = **222 passed / 0 failed**; `cgen_cemitter_sync`
> **17/17** (byte-identity intact; +float-arith +multidim +hoist +entry +unknown-call +bare-return +dedup +arg-count); +4 interp Giant-context fixes; +8 cgen.x self-hosted-generator fixes this session (float-arith, multi-dim, hoisting, entry-point, unknown-call drop, dead-gosub-goto, function-dedup, arg-count) lifting **self-hosted demo faithfulness 1→44**; native bootstrap fixed point **intact**; LLVM
