...
> xb_files[256] table-full fclose+reject, xb_open NULL name guard. NULL-safety
> on all memcpy/memset/memcmp (if(n)/if(m) guards), all malloc/calloc/realloc
> (abort on OOM for xb_alloc/dynamic arrays, graceful-fail for quicksort/
> copyarray/back_to_bin), all byref output pointers, all const char* params,
> all localtime returns, all x87 FPU *y params. C string literal escaping in
> DATA string emission (c_escape helper for \, ", \n, \t, \r). xb_cjust negative
> width guard. xb_str_n/xb_from_cstr/xb_back_to_bin zero-size memcpy guards.
> calloc NULL checks in dynamic array DIM emission. Raw string literal fix in
> xb_fmt_float. Bit operations (EXTS/EXTU/CLR/SET/MAKE) clamped to valid
> width 0-32 / offset 0-31 range in interpreter, C runtime, and cgen.x.
> Interpreter string builtins (CHR$/NULL$/SPACE$/HEXX$/BIN$/OCT$/CJUST$/RJUST$/LJUST$)
> guarded against negative integer args. MID$ len saturating_add. ATTACH row
...
>
> `cgen_cemitter_sync` is **63/63**, the positive corpus is **81/81
...
> — DCOMPLEX and SCOMPLEX-returning functions now work: DCCONJ/DCSIN/DCCOS/
> DCCOSH/DCSINH/DCEXP/DCLOG/DCSQRT/DCRMUL/DCPOLAR/DCLOG10/DCTAN/DCTANH/
> DCPOWERCR/SCCONJ/SCSIN/SCCOS all verified with deterministic outputs,
> composite return types emit as C structs (`xb_dcomplex`/`xb_scomplex`),
...
| ~~LEGACY-LIB-BINDING-POLICY~~ | frontend + C backend + runtime | ✅ **done 2026-08-30** — 303 behavior checks prove user-defined functions take precedence: `pure_lib_behavior` (175) + `stateful_lib_behavior (128) link compiled `xb_user_*` weak exports with C harness and verify deterministic outputs from user-defined bodies (xma SINH/COSH/ACOS, xcm DCCONJ/DCSQRT, xst XstGetOSName/XstVersion$, etc.). CEmitter `is_defined_func` gate + interp `find_function` ensure user-defined precedence over native helpers. | — |
| ~~CORE-LIB-BEHAVIOR-GATE~~ | tests | ✅ **done 2026-08-30** — 303 behavior checks across `pure_lib_behavior` (175: xma 56, xcm 107, xut 1, xit 8, xdis 3) + `stateful_lib_behavior (128: xst getters, setters, string functions, exception mapping, byval RETURN paths). Non-Version$ functions behavior-locked via C harness linking compiled `xb_user_*` exports. | — |
...
| ~~ARY-STATUS-RECONCILIATION~~ | tests + docs | ✅ **done 2026-08-30** — Bounded ATTACH behavior test (`cemitter_attach_copy_semantics_match_interp`): verifies Cases 1–5 (row copy from 2D, row writeback to 2D, whole-array copy, element→scalar, scalar→element) produce identical output across interpreter and Rust CEmitter. CEmitter fixes: Case 3 now handles fixed-size stack arrays (not just dynamic), Case 1 ubound assignment gated on `is_dyn_array`, Case 2 copy size uses `sizeof` for fixed-size arrays. `collect_array_dims` extended to track 1-D fixed arrays. cgen.x excluded (text IR drops ATTACH as Nop). Sync 62/62, demo 27/27. | dynamic 2nd-dim arrays still no-op (no known corpus) |
...
| ID | Candidate | Status | Evidence | Next action / trigger |
|---|---|---|---|---|
| ~~L1~~ | Separate compile/link/behavior/production readiness wording | **done 2026-08-30** | 15/15 Rust compile/link; 303 behavior checks (pure 175 + stateful 128); 15/15 cgen.x compile floor. Readiness classes: (a) emit+cc-clean ✓, (b) multi-lib link ✓, (c) behavior of compiled legacy body ✓ (303 checks), (d) production runtime — open (GUI-RUNTIME, packaging). | — |
| ~~L2~~ | Native-vs-legacy call-binding policy before behavior-port claims | **done 2026-08-30** | `pure_lib_behavior` + `stateful_lib_behavior` tests link compiled `xb_user_*` exports (weak) with C harness; 303 checks verify user-defined bodies execute, not native helpers. `is_defined_func` gate in CEmitter + `find_function` in interp ensure user-defined precedence. | — |
| ~~L3~~ | Non-Version$ core-library behavior test | **done 2026-08-30** | 303 behavior checks: `pure_lib_behavior` (175: xma/xcm/xut/xit/xdis) + `stateful_lib_behavior` (128: xst, including XstGetOSVersion/XstGetPrintTab/XstGetSystemError via DECLARE byref) | — |
...
| reproduce locked compiler claims | yes, with named-test caveats | 81/81 corpus byte lock, 61/61 sync, `IR_IDENTICAL`; demo compile guard uses two post-emit rewrites |
...
| RR-11 | Licensing and shim provenance | **partial — notices resolved 2026-08-31; tracked port same day** | Root ships complete canonical GNU texts (`COPYING` GPL-2, `COPYING_LIB` LGPL-2.1 — upstream copies lacked GNU title/version/Preamble); `LICENSING.md` maps per-directory licenses (crates declare GPL-2.0+/LGPL-2.1+ per Cargo.toml). License-cleared source material now ships in the tracked `xbasic/` tree with its own canonical texts + per-file audit `xbasic/LICENSES.md` (see TRACKED-CORPUS-PORT); legacy versioned trees stay gitignored local-only. LGPL-derived C ports live in one canonical location (`crtl/`); superseded dated xlib snapshots isolated to `old-versions/` with provenance README. **Remaining (legal):** three shims (`gdi32`/`kernel32`/`user32`) carry no copyright/license statement — they ship in `xbasic/lib/` solely under the upstream release's tree-level distribution (flagged in LICENSES.md); do not redistribute separately, and `xblibs` stays internal-test-only until distribution obligations cleared | resolve shim provenance and distribution obligations before packaging |
...
> frontend 24/24, compiler 68/68, behavior 303 checks (175 pure + 128 stateful), demo 27/27, sync 63/63.
