...
| RR-11 | Licensing and shim provenance | **partial — notices resolved 2026-08-31** | Root now ships complete canonical GNU texts (`COPYING` GPL-2, `COPYING_LIB` LGPL-2.1 — upstream copies lacked GNU title/version/Preamble); `LICENSING.md` maps per-directory licenses (crates declare GPL-2.0+/LGPL-2.1+ per Cargo.toml, legacy trees gitignored local-only reference); LGPL-derived C ports isolated to single canonical location `xbasic-6.4.5/src/crtl/` (lib/ stubs reduced to pure references, no duplicated code); superseded dated xlib snapshots isolated to `lib/old-versions/` with provenance README. **Remaining (legal):** three shims (`gdi32`/`kernel32`/`user32`) carry no copyright/license statement — provenance unresolvable from repo evidence; `xblibs` stays internal-test-only until distribution obligations cleared | resolve shim provenance and distribution obligations before packaging |
