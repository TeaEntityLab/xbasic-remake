...
> test --workspace --exclude xb-ide` = **218 passed / 0 failed**; `cgen_cemitter_sync`
> **13/13** (byte-identity intact; +computed-GOTO +AT-write +pos/SIZE +giant-literal +float-arith +multidim +undeclared-local-hoist +non-Main-entry); +4 interp Giant-context fixes; +cgen.x float-arith, multi-dim, hoisting & entry-point fixes; native bootstrap fixed point **intact**; LLVM
...
| CGEN-SELFHOST-PARITY | self-hosted cgen.x | **The demo sweep/differential measure the Rust `CEmitter` (`--compile`), not cgen.x.** cgen.x's *true* demo faithfulness (build native cgen from cgen.x, then IR→cgen→cc→run vs interp over 114 demos) was **faithful=1, cc-fail=112** — cgen.x is a **minimal self-hosting seed** that only ever ran on the selfhost tools (subset: `Main` entry, every local DIM'd, no unknown builtins, no shared arrays). Now **faithful=7, diverge=0, cc-fail=107**. Fixed: **(0) ~~entry point~~ ✅ (`8484a72`)** — cgen.x called only `xb_user_Main()`, but all 114 demos use a non-`Main` entry (`FUNCTION Entry`), so it called *nothing* → empty output; now calls `Main` else the first parameterless function (mirrors `entry_or_first`), flipping faithful 3→7 / diverge 4→0. **(1) ~~undeclared-local hoisting~~ ✅ (`d98c635`)** — mirrors `emit_hoisted_scalars` (collect `symbol`/`for`/`assign` scalars minus DIM/REDIM/params/retval → prologue decls); byte-neutral (selfhost has 0 undeclared locals), locked by `cemitter_and_cgen_agree_on_undeclared_local`. Both byte-neutral on selfhost (sync 13/13, bootstrap intact). Remaining: **(2) UNKNOWN-BUILTIN decls — the dominant wall (~61 Xui/Xgr GUI + ~24 Xst)**: undeclared-builtin calls emit `xb_user_Xxx(...)` with no forward-decl → implicit-decl cc error (CEmitter stubs them; real value needs RT-XST/GUI-RUNTIME, not just stubs). **(3) SHARED arrays** — CGEN-SHARED-ARR-SELFHOST. Latent: interp-vs-**both-C-backends** FOR-final-value gap (post-loop counter = limit+step in C, = limit in interp). Next: gap (2). | self-hosted demo AOT | infra |
