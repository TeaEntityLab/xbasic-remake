...
> test --workspace --exclude xb-ide` = **204 passed / 0 failed**; `cgen_cemitter_sync`
...
| CORE-LIBS-CC | C backend | `src/` system libs cc-clean: **11/18 compile** (was 6; +kernel32/MakeDistLinux/xutpde/CreateHelp/xrun this session). The 7 fails each bottom out on the **hardest dual-use / collision codegen** (each has a tail of errors; a lib flips only when all clear; libs are EXTERNAL-bottomed → cc-clean bar, no runtime differential): **xin** — `SHARED SOCKET socket[]` is a *dual-use SHARED composite array* AND a function name (`socket()` syscall) AND a scalar (`socket = slot`); its array facet isn't `_arr`-tagged (`socket[socket]`) + `host.address` composite member typed pointer-vs-scalar. **xdis** — module-level `XLONG imm` + `STRING imm$` VAR-SUFFIX-COLLISION; the int facet `xb_var_imm` isn't hoisted. **xst/xit/xgr/xui/xcol** — 20+ tails of undeclared composite/`_s`-variant/dual-use names + `subscripted value is not an array` (scalar subscripted). All in the delicate dual-use path that regressed CGEN-SHARED-ARR's first attempt → each is a careful per-lib pass, not a general fix. Foundational for *real* GUI-RUNTIME via IMPORT-linking. | core libs migration | codegen (large, per-lib) |
