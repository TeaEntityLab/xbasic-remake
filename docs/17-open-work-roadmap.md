...
| ~~RR-08a~~ | Behavior gates for pure libraries | **done 2026-08-30** — `pure_lib_behavior` test compiles xma.x + xcm.x + xut.x + xit.x via CEmitter and verifies 131 deterministic outputs from compiled legacy bodies: xma (36: SINH/COSH/TANH at 0+1+2, ACOS/ASIN/ATANH/LOG10/ACOSH/ACOT/ACOTH/ACSC/ACSCH/ASEC/ASECH/COTH/CSC/CSCH/SEC/SECH/LOG/XmaVersion$), xcm (87: XcmVersion$/Atan2/DCABS/DCARG/DCNORM + all 22 DCOMPLEX + all 19 SCOMPLEX-returning functions), xut (1: XutInit), xit (2: XitVersion$/Welcome), xst (5). CEmitter fix: EXTERNAL FUNCTION declarations for recognized builtins now map to C runtime helpers. Gates: sync 61/61, demo regression 27/27 | xdis (disassembler) behavior; COT (depends on FPU stub XxxFPTAN) |
| ~~RR-08b~~ | Behavior gates for stateful libraries | **done 2026-08-30** — `stateful_lib_behavior` test compiles xst.x via CEmitter and verifies 8 deterministic outputs: XstGetOSName(@name$)="linux unix", XstGetConsoleGrid(@grid)=0, XstVersion$()="6.4.5", XstGetEndianName(ret)=0, XstGetCPUName(ret)=0, XstGetApplicationEnvironment(@standalone,@reserved)=(0,0,0). Tests byref string/int output params, SHARED variable reads, SELECT CASE lowering, RETURN value path. Gates: sync 61/61, demo regression 27/27 | extend to xui/xgr/xcol; bounded ARY evidence |
