...
| CGEN-REDIM / CGEN-BYREF-REDIM | C backend | `REDIM` emitted as `DIM` (fixed C arrays; no resize-preserving realloc). **REDIM-through-`@array[]` CRASHES cgen — verified 2026-08-21** (`FUNCTION Grow(@a[]) { REDIM a[4] }` called with `@n[]`: interp → `ubound=4, n3=99`; cgen → SIGABRT rc=134). Root: by-ref arrays pass as a **raw pointer** `T* xb_var_a`, so a callee `REDIM` (realloc) can't update the caller's pointer or length → use-after-free. Needs the array ABI: pass a `{data, len/dims}` heap descriptor **by pointer** so resize propagates. **This is the blocker for RT-XST's `XstQuickSort` (14 uses — REDIMs the index array `@n[]` to match `a[]`) and `XstCopyArray` (2 uses).** The number parser `XstStringToNumber` needed no arrays and is done; these two need this foundational unblock (analogous to how GIANT unblocked the parser) | `XstQuickSort`/`XstCopyArray`; content-preserving REDIM | feature (foundational) |
