...
| CORE-LIBS-CC | C backend | `src/` system libs cc-clean: **13/18 compile** (was 12; +xin). Session 2026-08-23 landed 7 fixes (total errors 80→28): **CGEN-SHARED-COMPOSITE** (shared composite array members as globals), **CGEN-TYPEAWARE-HOIST** (params/dimmed keyed by `(name, is_str)`), **CGEN-DUALUSE-SCALAR-PARAM** (scalar params used as arrays get dual-use split), **CGEN-SHARED-DIM-EXCLUDE** (shared DIMs excluded from scalar noting), **CGEN-AUTOSYM-DOLLAR** (resolve `$`-suffixed names to base name in symbol table when base is String), **CGEN-STRPARAM-DUALUSE** (string param + integer array DIM type-aware dual-use), **CGEN-SHARED-ARR-NO-ARR** (skip `_arr` suffix for shared array globals + REDIM type suffix fix), **CGEN-OCTAL-LITERAL** (`0o` prefix octal literals in C emitter). Error counts: xcol=2, xit=7, xst=13, xin=0, xgr=6, xui=0. Remaining categories: xcol (SELECT CASE exit label + ATTACH cross-function array), xit (own_name hoist skip + SELECT CASE exit label + label redefinition + string-as-array-index), xst (ARGV$/OSERROR$ built-in shared vars + ATTACH + array subscript type + descriptor `_d` vs Double `#`→`_d` collision), xgr (dual-use shared vars like borderWidth). Byte-neutral: sync 46/46, demo regression 27/27. | 1 core lib (xin) | ⏳ |
