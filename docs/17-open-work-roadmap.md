...
| CGEN-SELFHOST-PARITY | self-hosted cgen.x | **The demo sweep/differential measure the Rust `CEmitter` (`--compile`), not cgen.x.** cgen.x's *true* demo faithfulness (build native cgen from cgen.x → IR→cgen→cc→run vs interp, 114 demos) was **faithful=1** — cgen.x was a **minimal self-hosting seed**. Now **faithful=98, diverge=0 (every compilable demo is byte-faithful!), cc-fail=15, interp-timeout=1** (2026-08-22: +nested-fn, +1-D dyn-array family, +scalar-DIM dedup, +undimmed/UBOUND folds, +per-function cross-function scoping `8a6e99d`, +forward-ref-scalar-hoist & C-name type-collision-dedup `f7bc9ec` flipping xgrids, +scalar-string-DIM-of-dyn-string-array skip flipping aprofile+adata, +SHARED-array heap-globals & undeclared-shared-scalar decl flipping amemory+amakemap, +`&func` decl-order id via `##funcIds$` flipping atimer, +2-D fixed+dyn arrays (`scan_arr2d$`/`emit_flat2d$`/`emit_d1$`), +typed fixed-array element type via `c_type$` `91c58dd`, +**UBOUND(s$)=LEN-1** `690cbce` **flipping aback**, +**byte-accurate NUL strings** (`xb_str_n` literal w/ conditional helper + `fwrite` non-last PRINT) `8a63862` **flipping atrim**, +**i32 mask + do-while loops** `f49bd0a` **flipping acrc32** (integer arith/bitwise/unary + hex/binary literals wrap `(int32_t)`; DO...LOOP UNTIL/WHILE emit `do{}while()` not the old `while(1){}while()` infinite-loop; both byte-neutral on the corpus, addresses stay intptr_t) → **diverge=0**; the 15 cc-fails need ANY-array+ATTACH (aarray/aarray_ISNODE), case-B, composite, or network — verified multi-feature, not bounded)
...
| CGEN-EXPR-GAPS | self-hosted cgen.x | A **completeness audit** of cgen.x `emit_expr$` vs every `IrExprKind`. **`funcaddr(` (`&Func()`) ✅ DONE** (verified 2026-08-22): CGEN-FUNCADDR (`funcaddr(` arm + program-order `##funcIds$`) handles both the arg form (`atimer` faithful, uses it 3×) and the scalar-assign form (`p = &Helper() : IF p<>0` → interp/cgen.x both `nonzero 2`, byte-identical). **`byref(` scalar (`@x` by-ref, `ByRef`) — still open, CONFIRMED gap** (2026-08-22): the `byref(` arm falls through to `"0"` for a non-byref-dual scalar, so `Bump(@x)` passes `0` → cgen.x prints `5`, interp/Rust print `6`. **Byte-neutral** (self-host tools have 0 scalar-byref user-calls) and **0-flip** (no faithful demo uses it — all byref demos are also array-descriptor-blocked), so it's a pure completeness fix. Scoped as an 8-edit CGEN-BYREF pass: call-site-driven by-ref-param scan (`:F\|P:`), `##emitFnName$`/`##curByrefParams$` context, pointer param decls in `emit_params$`, `&arg` (or the pointer, if the caller's arg is itself a by-ref param) at the `byref(` call arm, and deref (`(*xb_var_X)`) at the scalar symbol-read + assign-target sites. Statement dispatch is otherwise complete (LEFT$/MID$ offsets audited clean). | drift blind spot | ◑ funcaddr done; byref scoped |
