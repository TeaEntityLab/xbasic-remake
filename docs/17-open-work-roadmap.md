...
> The same-day unsized-array/facet slice preserves `DIM a[]` in Text IR,
...
> The follow-on allStrArr facet-driven migration slice makes cgen.x consume
> facet headers for `##allStrArr$` when facets are present (falling back to
...
> `checks/validate-all.sh` passes **315 tests across 40 binaries**;
> `checks/verify-bootstrap.sh` is green, including `cgen_cemitter_sync`
> **70/70**. The LLVM feature gate passes **144 tests** with one documented
...
> sync 64/64, positive corpus 81/81, demo regression 27/27,
...
> The last full workspace run reports **308 passed / 0 failed across 33 binaries**. In
...
>
> `cgen_cemitter_sync` is **64/64**, the positive corpus is **81/81
...
| **done** | **~~RR-06 ATTACH~~ copy-semantics runtime (2026-08-30)** | 5 ATTACH patterns in interpreter + Rust CEmitter; per-dim size vars at 2D DIM; guarded no-op for dynamic 2nd-dim; sync 64/64, demo regression 27/27 |
...
| ~~ARY-STATUS-RECONCILIATION~~ | tests + docs + frontend + cgen.x + LLVM | ✅ **done 2026-08-30; three-engine 2026-09-03; LLVM 2026-09-04** — Bounded ATTACH behavior test (`cemitter_attach_copy_semantics_match_interp`): Cases 1–5 (row extract/restore, whole-array copy, element→scalar, scalar→element) produce identical output across interpreter, Rust CEmitter, and selfhosted cgen.x (17-line lock incl. string scalar/element moves and string whole-array copy). cgen.x mirrors `c_emit_attach.rs` in `emit_stmt$` (`attach_split$`/`attach_side_parse$`/`attach_row_len$` helpers): dyn/shared rows copy flat via `xb_d1_` stride, fixed 2-D rows via native `&a[i][0]` base (flat indexing overruns dim 0), fixed-fixed whole arrays via `sizeof`-min `memcpy`, strings via `xb_strdup`. Frontend fix: `A$[]` carries `has_brackets` through `Statement::Attach` so whole-array `$` operands keep their suffix for array-slot resolution (was mistyped `a:integer`, silent no-op in all engines). Text IR `attach L TO R` round-trips (`round_trip_attach_five_cases`). Dynamic trailing-comma DIM locked (`cemitter_and_cgen_agree_on_dynamic_trailing_comma_dim`): lowering normalizes to 1-D everywhere, row-ATTACH stays a guarded no-op. LLVM backend lowers all five cases (`emit_attach`/`emit_copy_loop`: holder+count model, `dyn_resize` growth, branchless clamped lengths; same-rank whole-array, shallow string copies like its own assignment) locked by `llvm_backend_attach_copy_semantics` on the same 17 lines (was `_ => {}` silent no-op). Sync 77/77, 15/15 libs cc-clean, LLVM feature gate green. | REDIM-across-fixed content preservation still differs (interp/Rust dyn-promote and preserve; cgen.x keeps fixed stack storage) — pre-existing, verified identical before ATTACH work |
...
> frontend 26/26, compiler 77 (70 lib + 4 dec + 1 legacy + 2 source), behavior 340 checks (175 pure + 165 stateful), demo 27/27, sync 64/64.
