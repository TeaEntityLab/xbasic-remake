...
lever left**. The 6 interpreter-clean programs that still diverge, by root cause
(byte-faithful PRINT output, `MID$`/`s${n}=v` byte-assignment, the auto-vivified-scalar
prealloc, `$$` constant evaluation, and the `OPEN`/`CLOSE`/`LOF` file runtime were resolved
this session; see the summary):

| Blocker | Programs | Nature |
|---|---|---|
| File / record I/O | `acrc32`, `arecord` | `OPEN`/`CLOSE`/`LOF` are implemented (libc `fopen`/`fclose`/`fseek`/`ftell` + a `FILE*` handle table; unblocked `astring`). What remains is the **byte/record data path** — native `READ [n]`/`WRITE [n]` (`acrc32`, via stdin `INLINE$`) and fixed-record `GET`/`PUT` (`arecord`) — plus `acrc32`'s degenerate `OPEN("")` on empty stdin |
...
**99/105 faithful, 0 compile-fails** (byte-strings resolved RT-BYTESTRING; byte-faithful
...
constant evaluation + the `OPEN`/`CLOSE`/`LOF` file runtime, unblocked `astring`; `FORMAT$` +
...
unblocked `aarray_ISNODE`). The residual 6 are the large/fundamental efforts tabled above.
