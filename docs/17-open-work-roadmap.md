...
> test --workspace --exclude xb-ide` = **213 passed / 0 failed**; `cgen_cemitter_sync`
> **8/8** (byte-identity intact; +computed-GOTO +AT-write +pos/SIZE); +4 interp Giant-context fixes (neg/bitwise/conditions/builtin-args); native bootstrap fixed point **intact**; LLVM
...
| GIANT-LITERAL | cgen.x PRINT narrowing | **sub-bug (2) ✅ fixed** (`9b829f1`): the interp now coerces a Giant arg to an Integer-param builtin (STR$/ABS/…) to i32 via `xb_compiler::builtin_param_types`, matching the C backends (was a crash; a clean signature-driven fix, not the feared broad refactor). **Remaining sub-bug (1)**: cgen.x narrows bare Giant literals in PRINT/arith — a SYNC DRIFT (`PRINT 2147483648` → cgen.x `-2147483648` vs interp/rust `2147483648`; `+1` → `-2147483647` vs `2147483649`). cgen.x's `integer(V)` handler + integer print treat the value as 32-bit, while Rust promotes it to a 64-bit Giant via the parsed value_type. Uncaught (no corpus/demo uses i32-overflowing literals). Fix: cgen.x detect i32-overflow-by-value → emit/print 64-bit matching Rust (bootstrap-critical; verify byte-neutral on the 0-Giant-literal corpus). Repro: `PRINT 2147483648` | Giant-literal PRINT faithfulness | infra |
