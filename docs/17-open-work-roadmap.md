...
> test --workspace --exclude xb-ide` = **199 passed / 0 failed**; `cgen_cemitter_sync`
...
| CORE-LIBS-CC | C backend | `src/` system libs cc-clean: **9/18 compile** (was 6; +kernel32 via CGEN-COMPOSITE-MEMARR-TYPE, +MakeDistLinux/xutpde via CGEN-SHARED-IF-COND). 9 fail with distinct emit bugs, all parse+emit-C fine: **builtin arg-count** (CreateHelp, xcol: 2 args to a 1-arg builtin); **undeclared `xb_var_*`** (xdis `imm`, xin `time`, xui `window`); **undeclared `xb_str_*`** composite/string member (xit `dbase`, xst `fileInfo_fileName`, xgr `display_s`); **unary type** (xrun: `char*` to a unary op). Libs are EXTERNAL-bottomed (not runnable) → bar is cc-clean, no differential. Foundational for *real* GUI-RUNTIME via IMPORT-linking (xgr/xui/xst are the demos' Xui*/Xgr* impls). | core libs migration | codegen (large) |
