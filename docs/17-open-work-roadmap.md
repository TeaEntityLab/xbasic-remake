...
> test --workspace --exclude xb-ide` = **212 passed / 0 failed**; `cgen_cemitter_sync`
> **8/8** (byte-identity intact; +computed-GOTO +AT-write +pos/SIZE); +3 interp Giant-context fixes (neg/bitwise/conditions); native bootstrap fixed point **intact**; LLVM
...
| GIANT-LITERAL | two independent sub-bugs | i32-overflowing literals handled inconsistently (uncaught — no corpus/demo uses them; the 3 *common* Giant-context bugs are already fixed under RT-GIANT-CTX). **Rust is correct + consistent**: a bare Giant literal is preserved 64-bit; a Giant coerced to an Integer builtin param is narrowed (context-dependent, right). Two OUTLIER bugs, each should match the other two backends, both independent: **(1) cgen.x narrows bare Giant literals in PRINT/arith** — SYNC DRIFT (`PRINT 2147483648` → cgen.x `-2147483648` vs interp/rust `2147483648`); cgen.x's `integer(V)` emit_expr$ treats the token as 32-bit, needs i32-overflow-by-value detection → 64-bit (bootstrap-critical). **(2) interp errors on a Giant arg to any Integer-param builtin** — a CLASS (`STR$`, `ABS`, …; rust+cgen narrow); `call.rs:322` passes raw args, needs signature-driven Giant→i32 coercion (broad refactor). Both niche; fresh runway. Repro: `PRINT 2147483648`, `PRINT STR$(2147483648)` | Giant-literal faithfulness | infra |
