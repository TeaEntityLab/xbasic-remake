...
| GIANT-LITERAL | all backends | **Giant-literal handling is inconsistent** (uncaught — no corpus/demo uses i32-overflowing literals). The text IR emits a 2^31-overflowing literal as `integer(V)` with no Giant marker; the Rust CEmitter recovers the Giant via the parsed value_type, cgen.x sees only the token. **Sync drift**: `PRINT 2147483648` → interp=rust `2147483648` (Giant, 64-bit) vs cgen.x `-2147483648` (narrows to 32-bit); `+1` → `2147483649` vs `-2147483647`. Also `STR$(2147483648)` errors in the interp while rust+cgen narrow — and rust is internally inconsistent (PRINT preserves the Giant, STR$ narrows). Needs a canonical decision (promote-to-Giant vs wrap-to-XLONG) + multi-backend alignment (mark Giant literals in the text IR, or cgen.x detect i32-overflow by value + reconcile rust PRINT-vs-STR$). Bootstrap-critical → fresh runway. Repro: `PRINT 2147483648` | Giant-literal faithfulness | infra |
