...
> --exclude xb-ide` = **189 passed / 0 failed**; `cgen_cemitter_sync` **5/5**;
...
| RT-XST | interp + backends | real `XstStringToNumber`/`XstQuickSort`/`XstCopyArray` (all zero-stubs today). **⛔ BLOCKED (2026-08-21) on the frozen GIANT type model.** `XstStringToNumber(s$, @start, @after, @rtype, @value$$)` writes `value$$` (a GIANT) — for a parsed DOUBLE it holds f64 bits, for a >32-bit literal a true 64-bit int. But `RuntimeValue` is `Integer(i32)\|Float(f64)\|String` and `ValueType::from_suffix` maps GIANT (`$$`) → `Integer` (i32) — no 64-bit slot. A *faithful* impl needs a real `Giant(i64)` value type, but the **frozen v0.1 goldens bake in GIANT≈i32** (`bit_ops`/`bit_reinterp`/`type_conv_rotate`/`system_variables` use `$$`/GLOW/GHIGH under 32-bit masking) — splitting GIANT into i64 changes their arithmetic/bit-ops/printing → rewrites frozen goldens (forbidden), and also perturbs the bootstrap corpus (cgen.x/compiler.x use `$$`). A partial i32-only `value$$` would silently corrupt float + 64-bit parsing (a fake impl). Ref is x86_64 asm only (`xbasic-6.4.5/src/linux/lib/xlib.s`; no C impl). Golden-safe to *add* (no v0.1/bootstrap program *calls* Xst*), but unfaithful to *implement* until GIANT is 64-bit. Libs that need it (msc/fgr/vgr via `Msc*` wrappers, qbtoxb) currently compile+match as mutual 0-stubs | `ary`'s `-48000`; msc/fgr/vgr functional parsing | feature (blocked) |
