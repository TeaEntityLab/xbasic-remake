...
| ~~RR-08a~~ | Behavior gates for pure libraries | **done 2026-08-30** — `pure_lib_behavior` test compiles xma.x + xcm.x via CEmitter and verifies 73 deterministic outputs from compiled legacy bodies: xma (12: SINH/COSH/TANH/ACOS/ASIN/ATANH/LOG10/XmaVersion$), xcm (55: XcmVersion$/Atan2/DCABS/DCARG/DCNORM + all 22 DCOMPLEX-returning functions: DCCONJ/DCSIN/DCCOS/DCCOSH/DCSINH/DCEXP/DCLOG/DCSQRT/DCRMUL/DCPOLAR/DCLOG10/DCTAN/DCTANH/DCPOWERCR/DCACOS/DCASIN/DCATAN/DCPOWERCC/DCPOWERRC + 3 SCOMPLEX: SCCONJ/SCSIN/SCCOS), xut (1: XutInit), xst (5). DCOMPLEX and SCOMPLEX composite return types fully working including composite return → composite param chaining (DCPOWERCC: DCEXP(n * DCLOG(z)), DCPOWERRC: DCEXP(DCRMUL(n, LOG(z)))). Gates: sync 61/61, demo regression 27/27 | xdis (disassembler) behavior; more SCOMPLEX functions |
