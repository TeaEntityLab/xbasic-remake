...
> test --workspace --exclude xb-ide` = **223 passed / 0 failed**; `cgen_cemitter_sync`
> **18/18** (byte-identity intact; +float-arith +multidim +hoist +entry +unknown-call +bare-return +dedup +arg-count +inline-helper); +4 interp Giant-context fixes; +9 cgen.x self-hosted-generator fixes this session (float-arith, multi-dim, hoisting, entry-point, unknown-call drop, dead-gosub-goto, function-dedup, arg-count, INLINE$ helper) lifting **self-hosted demo faithfulness 1→47**; native bootstrap fixed point **intact**; LLVM
