...
| CGEN-SELFHOST-PARITY | self-hosted cgen.x | **The demo sweep/differential measure the Rust `CEmitter` (`--compile`), not cgen.x.** cgen.x's *true* demo faithfulness (build native cgen from cgen.x → IR→cgen→cc→run vs interp, 114 demos) was **faithful=1** — cgen.x was a **minimal self-hosting seed**. Now **faithful=97, diverge=1 (only acrc32 — interp i32-CRC value + a slow/hang; the deferred CGEN-SHIFT i32-mask mirror is broad), cc-fail=15** (2026-08-22: +nested-fn, +1-D dyn-array family, +scalar-DIM dedup, +undimmed/UBOUND folds, +per-function cross-function scoping `8a6e99d`, +forward-ref-scalar-hoist & C-name type-collision-dedup `f7bc9ec` flipping xgrids, +scalar-string-DIM-of-dyn-string-array skip flipping aprofile+adata, +SHARED-array heap-globals & undeclared-shared-scalar decl flipping amemory+amakemap, +`&func` decl-order id via `##funcIds$` flipping atimer, +2-D fixed+dyn arrays (`scan_arr2d$`/`emit_flat2d$`/`emit_d1$`), +typed fixed-array element type via `c_type$` `91c58dd`, +**UBOUND(s$)=LEN-1** `690cbce` **flipping aback**, +**byte-accurate NUL strings** (`xb_str_n` literal w/ conditional helper + `fwrite` non-last PRINT) `8a63862` **flipping atrim**; the 15 cc-fails need ANY-array+ATTACH (aarray/aarray_ISNODE), case-B, composite, or network — verified multi-feature, not bounded)
