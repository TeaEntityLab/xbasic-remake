...
| CGEN-MULTIDIM | C backend | cgen ignores `extra_dims`/`extra_indices` entirely (grep-confirmed 2026-08-20): every array is 1D. `DIM arr[3,2,]` allocates only `[3]` (`xb_ub_arr=3`); the interp flattens row-major to `(3+1)(2+1)-1=11`, so `UBOUND(arr[3,2,])` = 11 (interp) vs 0 (cgen), and `arr[i,j]` drops `j`. Kittedy/adatadim are faithful *only* because their 2D usage survives the 1D approximation — so proper flattening (alloc `∏(dk+1)`, index `Σ ik·stride`) risks regressing them and won't flip aarray (still needs ATTACH). A deliberate feature with regression risk, not a reactive fix | (latent; aarray) | feature |
