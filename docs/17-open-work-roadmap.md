...
> test --workspace --exclude xb-ide` = **220 passed / 0 failed**; `cgen_cemitter_sync`
> **15/15** (byte-identity intact; +float-arith +multidim +undeclared-local-hoist +non-Main-entry +unknown-call-drop +bare-return-no-gosub); +4 interp Giant-context fixes; +6 cgen.x self-hosted-generator fixes this session (float-arith, multi-dim, hoisting, entry-point, unknown-call drop, dead-gosub-goto) lifting **self-hosted demo faithfulness 1→44**; native bootstrap fixed point **intact**; LLVM
...
| CGEN-SELFHOST-PARITY | self-hosted cgen.x | **The demo sweep/differential measure the Rust `CEmitter` (`--compile`), not cgen.x.** cgen.x's *true* demo faithfulness (build native cgen from cgen.x → IR→cgen→cc→run vs interp, 114 demos) was **faithful=1** — cgen.x was a **minimal self-hosting seed** (selfhost subset: `Main` entry, every local DIM'd, no unknown builtins/shared arrays). Now **faithful=44, diverge=1 (amerge), cc-fail=68** after 5 byte-neutral fixes (sync 15/15, bootstrap intact): **(0) ~~entry point~~ ✅ (`8484a72`)** call `Main` else first parameterless fn. **(1) ~~undeclared-local hoisting~~ ✅ (`d98c635`)** mirror `emit_hoisted_scalars`. **(2) ~~unknown-call drop~~ ✅ (`b4e9413`)** external `Xui*`/`Xgr*`/`Xst*` → no-op/`0` like `is_unknown_call` (was the ~85-demo wall). **(3) ~~dead gosub-return goto~~ ✅ (`187eecc`)** — bare RETURN's `goto *xb_gosub_stack` in a no-`&&label` function is illegal C; rewrite to plain `return` when the body has no `&&` (was 16 demos, but most have further blockers). Locked by 6 `cemitter_and_cgen_agree_on_*` tests. Remaining cc-fails (68): 16 SHARED-array (CGEN-SHARED-ARR-SELFHOST); ~4 `xb_inline` (INLINE$ maps to `xb_inline`, C prelude omits the helper Rust emits); a **function-redefinition** cluster (a `DECLARE`d fn emitted twice, or dual-use name) + **arg-count** ("too many arguments" — cgen.x lacks the CEmitter's `emit_call_args` arity reconciliation); ~5 undeclared (hoisting edge cases: composite members / forms `scan_used$` misses). All are further Rust-CEmitter features unmirrored into cgen.x. | self-hosted demo AOT | infra |
