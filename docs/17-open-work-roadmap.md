...
| CGEN-SELFHOST-PARITY | self-hosted cgen.x | **The demo sweep/differential measure the Rust `CEmitter` (`--compile`), not cgen.x.** cgen.x's *true* demo faithfulness (build native cgen from cgen.x → IR→cgen→cc→run vs interp, 114 demos) was **faithful=1** — cgen.x was a **minimal self-hosting seed**. Now **faithful=90, diverge=3 (aback/atrim byte-level + acrc32 interp-CRC-bug, not cgen.x faults), cc-fail=20** (2026-08-22: +nested-fn, +1-D dyn-array family, +scalar-DIM dedup, +undimmed/UBOUND folds, +per-function cross-function scoping `8a6e99d`, +forward-ref-scalar-hoist & C-name type-collision-dedup `f7bc9ec` flipping xgrids)
