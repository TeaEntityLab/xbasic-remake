...
| CGEN-NDIM | cgen.x | Multi-dim (3+) arrays: cgen.x only emits 2D flat indexing (`emit_flat2d$` + single `xb_d1_` bound var); 3-index accesses emit garbage `[(i1)*(xb_d1_+1)+(i2),i3]` (comma-operator accident — adatadim "passed" earlier sweeps only via `-Wno-implicit-function-declaration`). Fix: per-dim bound vars at the DIM site + general N-dim flat renderer across access/assign/swap sites. Rust emitter already correct (`emit_flat_size`). | adatadim via cgen.x | feature |
