...
| CORE-LIBS-CC | C backend | `src/` system libs cc-clean: **17/18 compile** (was 13; +xcol, xgr, xst, xui). Session 2026-08-23 landed 6 fixes (total errors 28→2, all in xit): **CGEN-EXIT-SELECT-TRUE** (replace EXIT SELECT with GOTO label in desugared SELECT CASE TRUE), **CGEN-STRUBOUND-HOIST** (hoist string scalars used in UBOUND), **CGEN-CROSSFN-DUALUSE** (detect cross-function dual-use shared arrays — borderWidth scalar in fn A, array in fn B), **CGEN-BYTE-ACCESS** (handle XBasic `{}` byte-access operator on strings — `text$[l]{n}` → `xb_str_text_s_arr[l][n]`), **CGEN-DESC-SUFFIX** (rename descriptor `_d` suffix to `_dd` to avoid `#`→`_d` collision), **CGEN-OWNNAME-INT** (declare return var for Integer functions that use own name as variable). **Clean: xcol, xin, xgr, xst, xui (+ 11 others). Remaining: xit 2 errors** (byref-array descriptor ×1, nested SUB label collision ×1). | xit | ◑ |
