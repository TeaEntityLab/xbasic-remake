...
| CGEN-SHIFT | C backend | cgen values are `intptr_t` (i64); the interp is **i32** (`RuntimeValue::Integer(i32)`, `wrapping_shr` = arithmetic i32 shift). acrc32's `table[1]=0x09073096` (vs standard unsigned `0x77073096`) is the interp's signed-i32 arithmetic-shift result. **Experiment 2026-08-21** (reverted): `c_type(Integer)="int32_t"` + `-fwrapv` makes acrc32 **faithful via the CLI** and keeps the whole corpus byte-neutral (`cgen_positive_corpus` passes) with **72/114 demos faithful** (acrc32 flips, 0 new compile-fails) — the fix is *correct*. But it's a genuine **coordinated change**, not a one-liner: (1) `int32_t` signed overflow is UB, so it needs `-fwrapv` at **every** cc call — CLI ✓, plus 7 test files (`cgen_cemitter_sync`, `cgen_corpus`, `cgen_demo_regression`, `cgen_positive_corpus`, `cgen_selfhost`, `native_emit`, `native_pipeline`) and 2 bootstrap scripts (`checks/verify-bootstrap.sh`, `.github/workflows/bootstrap-verify.yml`) — without it `cgen_cemitter_sync` crashes; (2) a corpus program is i32/i64-divergent, so `cgen.x` must mirror the type map (`c_type$` 697 + integer decl/init sites 2142/2158/2251) while keeping address casts (`&&label` 1920, `xb_goaddr`/`xb_*at` runtime helpers, gosub stack) as `intptr_t` — bootstrap-critical, since a wrong site truncates a real pointer. The landed decision: keep it deferred rather than sprawl reactively across ~10 sites + a bootstrap-critical mirror | `acrc32` (diverge) | feature |
