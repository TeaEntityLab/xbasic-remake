...
| ~~SOURCE-COVERAGE~~ | tests + source | ✅ **done (2026-08-31)** — Every `.s` (assembly) file in `xbasic-6.4.5/src/` now has a `.x` or `.c` counterpart. 6 infrastructure `.s` files (appstart, xstart, xzzz, xlib, xlib230325, xlib230803) received `.c` stubs documenting the assembly and pointing to Rust toolchain replacements. `source_coverage.rs` test guard enforces: (1) every `.s` file has `.x`/`.c` counterpart, (2) every `libxb64.a` archive member has source counterpart. Prevents features from leaking into assembly-only files. |
