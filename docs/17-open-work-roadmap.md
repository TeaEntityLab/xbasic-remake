...
> test --workspace --exclude xb-ide` = **216 passed / 0 failed**; `cgen_cemitter_sync`
> **11/11** (byte-identity intact; +computed-GOTO +AT-write +pos/SIZE +giant-literal +float-arith +multidim); +4 interp Giant-context fixes; +cgen.x float-arith & multi-dim array fixes; native bootstrap fixed point **intact**; LLVM
...
> self-hosted `cgen.x`.** cgen.x is verified only on the selfhost tools (sync 11/11 +
...
| CGEN-SELFHOST-PARITY | self-hosted cgen.x | **The demo sweep/differential measure the Rust `CEmitter` (`--compile`), not cgen.x.** cgen.x's *true* demo faithfulness (build native cgen from cgen.x, then IR→cgen→cc→run vs interp over all 114 demos) is **faithful=1, diverge=1 (ahowdy), cc-fail=112** — cgen.x is a **minimal self-hosting seed**, not a full generator, and only ever runs on the selfhost tools (which are written to its subset: every local DIM'd, no unknown builtins, no shared arrays). Enumerated cgen.x-vs-CEmitter gaps, by demo-blocker frequency: **(1) UNDECLARED-LOCAL HOISTING — 91 demos** (dominant): XBasic auto-declares a scalar on first use; CEmitter hoists them (`c_emit_hoist.rs` `emit_hoisted_scalars`/`collect_dimmed` → `intptr_t xb_var_i = 0;`), cgen.x has **no hoisting** so `xb_var_i`/`xb_str_message` are undeclared → cc fail. **Fix is BYTE-NEUTRAL** (selfhost tools have 0 undeclared locals — verified: 0 cc errors compiling all 4 tools via cgen.x — so hoisting adds nothing there; bootstrap/sync unaffected) and would fix ~91 demos: mirror `emit_hoisted_scalars` into cgen.x (collect every `symbol(name:type)`/`array_access` ref in a function body, subtract DIM'd names + params, emit `c_type$(t) xb_var_name = default;` at the function prologue). **(2) unknown-builtin decls** — Xui*/Xgr*/Xst* calls emit `xb_user_Xxx(...)` with no forward-decl (implicit-decl → cc error under strict flags; CEmitter forward-declares/stubs them). **(3) SHARED arrays** — CGEN-SHARED-ARR-SELFHOST (row above). Recommend a dedicated bootstrap-gated pass starting with (1). | self-hosted demo AOT | infra |
