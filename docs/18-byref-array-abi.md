...
**Status (reviewed 2026-08-29, updated 2026-09-04 M1-COMP-UBOUND): LANDED for primitive/flat arrays, for shared composite `ARY_VAR_DATA` member arrays (compile-only), for typed composite-array by-ref reads + writeback (`FUNCTION Sum (PT p[])`, locked by `cemitter_and_cgen_agree_on_composite_array_byref`, `7/30/60`), and for callee-side member UBOUND at C-model parity (`cemitter_and_cgen_agree_on_composite_member_ubound`: C `0,0`, interp `1,1`). Still open: REDIM through a composite member, UBOUND real bounds in C (descriptor seeding, deferred), and plain forward-only callee-UBOUND callers without REDIM-seeded dyn (cc redefinition). Runtime `ARY` remains blocked on `ATTACH` alias semantics and a bounded behavior test (see docs/17 `RR-02`/`RR-06`).**
