...
**Status (reviewed 2026-08-29, updated 2026-09-04 M1-ATTACH-ALIAS): LANDED for primitive/flat arrays, for shared composite `ARY_VAR_DATA` member arrays (compile-only), for typed composite-array by-ref reads + writeback (`FUNCTION Sum (PT p[])`, `7/30/60`), for callee-side member UBOUND at C-model parity (C `0,0`, interp `1,1`), and for member REDIM through by-ref (`3/30/42/420/4`). DECIDED: real `ATTACH` is aliasing, not bounded copy — ary.x's extract→REDIM→writeback row growth cannot work under copy (row never grows; growth silently dropped), and the locked copy cases only assert immediates so they pass unchanged under alias. Interpreter reference landed (name links + REDIM-through-view row splice + DIM-unlink + SWAP-drop, four `attach_*` interp tests; calls pass value snapshots, links degrade to owned across boundaries). C backends still copy (documented divergence on aliasing-over-time) with a pointer + shared-ub alias-set design recorded; ary.x runtime follows the C work. Still open: UBOUND real bounds in C (descriptor seeding, deferred) and plain forward-only callee-UBOUND callers without REDIM-seeded dyn (cc redefinition — candidate `##redimNames$` design recorded, deferred). Runtime `ARY` remains blocked on the C-ATTACH work and a bounded behavior test (see docs/17 `RR-02`/`RR-06`).**
