...
**Status (reviewed 2026-08-29, updated 2026-09-04 M1-CATTACH): LANDED for primitive/flat arrays, for shared composite `ARY_VAR_DATA` member arrays (compile-only), for typed composite-array by-ref reads + writeback (`FUNCTION Sum (PT p[])`, `7/30/60`), for callee-side member UBOUND at C-model parity (C `0,0`, interp `1,1`), for member REDIM through by-ref (`3/30/42/420/4`), and for whole-array ATTACH aliasing on all three engines (heap pointers + bound cells; pointer-assign; guarded group REDIM with per-member repoint; `99/88/99/88` and `10/40/30/40/4/4` via `cemitter_attach_whole_array_aliases` + `cemitter_attach_group_redim_propagates`). Row ATTACH stays copy in C (interp aliases; immediate assertions agree). Still open: UBOUND real bounds in C (descriptor seeding, deferred), plain forward-only callee-UBOUND callers without REDIM-seeded dyn (cc redefinition — candidate `##redimNames$` design recorded, deferred), row-ATTACH aliasing in C, and ary.x runtime. Runtime `ARY` remains blocked on ary.x runtime proof (see docs/17 `RR-02`/`RR-06`).**
