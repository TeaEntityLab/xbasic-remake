# 22 — Open-Work History (archived narratives from docs/17)

> Lifecycle: frozen historical record. Moved out of docs/17 on 2026-09-06 under
> P10 / CAL-20260901-09 (separate historical snapshots from active state).
> Nothing here is an open row: open work, exit gates, and current evidence
> live only in docs/17; milestone sequencing in docs/20. Every count and
> status below is as of its own date and is superseded by the docs/17 active
> ledger where they differ.

## 1. Full re-verification snapshot narrative (2026-08-31)

> Kept verbatim from the docs/17 banner. The one-line anchor remains in docs/17.
...
## 2. Sections 1–4 as of 2026-08-17..20 (Backends, Runtime semantics, Frontend / migration coverage, Demos / GUI)

> Verbatim. Every item below was later closed, superseded, or re-rowed in the
> docs/17 execution queue and open-rows table (e.g. DEMO-RUNTIME 68/114 → raw
> 114/114 RR-13; XBSourceLib 9/13 → 11 faithful + 2 ARY compile-only;
> `ATTACH` deferred → move semantics RR-06).
