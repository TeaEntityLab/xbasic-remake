...
- [x] Add first runtime-backed interpreter for the typed IR subset: VERSION is metadata, DIM allocates a typed slot, assignment stores, PRINT appends to an output sink.
- [x] Add semantic support for function entry lookup (`Main`) and an interpreter test that executes `FUNCTION Main ... END FUNCTION`.
- [x] Add a minimal self-host utility written in XBasic source under `selfhost/` that the Rust CLI parses/analyzes/lowers successfully.
- [x] Add a self-host smoke test proving the Rust-hosted compiler pipeline accepts every `.x` file under `selfhost/`.
- [x] Document the completed Stage-0-to-Stage-1 bootstrap path in `docs/14-self-hosting-progress.md` with verifier evidence and remaining Stage-2 compiler-self-host tasks.
