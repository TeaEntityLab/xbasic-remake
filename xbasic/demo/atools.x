...
INTERNAL FUNCTION  Initialize         ( FUNCADDR func[], grid$[], image$[] )
...
'
	...
	XgrGetWorkArea (@waX, @waY, @waW, @waH)
...
'
SUB SwitchWindow
	IF (message$ <> "Selection") THEN EXIT SUB
...
' ###########################
'
FUNCTION  Initialize ( FUNCADDR func[], grid$[], image$[] )
'
	DIM func[31]
	DIM grid$[31]
	DIM image$[31]
...
'
	SELECT CASE message$
		...
		CASE "Notify"	    : GOSUB XuiDropButtonSelection
		CASE "Help"				: CallbackHelp (@v0$, @v1$, @v2$, @v3$, @xx$)
		CASE "Displayed"	: CallbackHelp (@v0$, @v1$, @v2$, @v3$, @xx$)
		CASE "Hidden"			: CallbackHelp (@v0$, @v1$, @v2$, @v3$, @xx$)
		CASE ELSE PRINT message$
	END SELECT
	RETURN
'
'
