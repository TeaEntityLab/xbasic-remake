...
SUB TextModified
	...
	IF (LEN(text$) > maxChars) THEN
		message$ = "[Too Many Characters]\nA maximum of " + STR$(maxChars) + " characters is allowed"
		XuiMessage (message$)
		text$ = LEFT$(text$, maxChars)
		XuiSendStringMessage (grid, "SetTextString", 0, 0, 0, 0, 0, @text$)
		XuiSendStringMessage (grid, "RedrawText", 0, 0, 0, 0, 0, 0)
	END IF
END SUB
'
' Selection message
