pub(crate) fn emit_math_functions(out: &mut String) {
    ...
    out.push_str("static char* xb_rclip1(const char* s) { int len = xb_len(s); while (len > 0 && isspace((unsigned char)s[len-1])) len--; char* r = xb_alloc((size_t)len); if (len) memcpy(r, s, (size_t)len); return r; }\n");
    out.push_str("static char* xb_rclip2(const char* s, int n) { int len = xb_len(s); if (n >= len) return xb_str(\"\"); int newlen = len - n; char* r = xb_alloc((size_t)newlen); if (newlen) memcpy(r, s, (size_t)newlen); return r; }\n");
    out.push_str("static char* xb_lclip1(const char* s) { int n = xb_len(s); int i = 0; while (i < n && isspace((unsigned char)s[i])) i++; int len = n - i; char* r = xb_alloc((size_t)len); if (len) memcpy(r, s + i, (size_t)len); return r; }\n");
    out.push_str("static char* xb_lclip2(const char* s, int n) { int len = xb_len(s); if (n >= len) return xb_str(\"\"); int nl = len - n; char* r = xb_alloc((size_t)nl); if (nl) memcpy(r, s + n, (size_t)nl); return r; }\n");
    out.push_str("static int xb_inchr(const char* s, const char* set, int start) { if (!set) return 0; int len = xb_len(s); for (int i = start - 1; i < len; i++) { if (strchr(set, s[i])) return i + 1; } return 0; }\n");
    out.push_str("static int xb_rinchr(const char* s, const char* set, int end) { if (!set) return 0; int len = xb_len(s); int begin = end - 1; if (begin >= len) begin = len - 1; for (int i = begin; i >= 0; i--) { if (strchr(set, s[i])) return i + 1; } return 0; }\n");
    out.push_str("static int xb_inchri(const char* s, const char* set, int start) { if (!set) return 0; int len = xb_len(s); char* lset = xb_strdup(set); for (char* p = lset; *p; p++) *p = (char)tolower((unsigned char)*p); for (int i = start - 1; i < len; i++) { if (strchr(lset, tolower((unsigned char)s[i]))) return i + 1; } return 0; }\n");
    out.push_str("static int xb_rinchri(const char* s, const char* set, int end) { if (!set) return 0; int len = xb_len(s); int begin = end - 1; if (begin >= len) begin = len - 1; char* lset = xb_strdup(set); for (char* p = lset; *p; p++) *p = (char)tolower((unsigned char)*p); for (int i = begin; i >= 0; i--) { if (strchr(lset, tolower((unsigned char)s[i]))) return i + 1; } return 0; }\n");
    out.push_str(
    ...
    out.push_str("static char* xb_mid2(const char* s, int start) { int slen = xb_len(s); if (start < 1) start = 1; int off = start - 1; if (off >= slen) return xb_str(\"\"); int len = slen - off; char* r = xb_alloc((size_t)len); if (len) memcpy(r, s + off, (size_t)len); return r; }\n");
    out.push_str("static char* xb_stuff(const char* into, const char* from, int start, int len) { int ilen = xb_len(into); int flen = xb_len(from); int si = start - 1; if (si < 0) si = 0; if (si > ilen) si = ilen; int avail = ilen - si; int max_from = (len < 0) ? flen : (len < flen ? len : flen); int p2 = max_from < avail ? max_from : avail; char* r = xb_alloc((size_t)ilen); if (si) memcpy(r, into, (size_t)si); if (p2) memcpy(r + si, from, (size_t)p2); if (ilen - si - p2) memcpy(r + si + p2, into + si + p2, (size_t)(ilen - si - p2)); return r; }\n");
    out.push_str(
