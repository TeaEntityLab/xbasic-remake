...
pub(crate) fn emit_xin_runtime(out: &mut String) {
    ...
    out.push_str(
        "static int64_t xb_xin_socket_bind(int32_t sock, int32_t block, int64_t* address, int32_t* port) { (void)block; (void)address; struct sockaddr_in a; memset(&a,0,sizeof a); a.sin_family=AF_INET; a.sin_addr.s_addr=htonl(INADDR_ANY); a.sin_port=htons(port ? (uint16_t)*port : 0); int r = bind(sock,(struct sockaddr*)&a,sizeof a); if(r==0){ socklen_t n=sizeof a; if(getsockname(sock,(struct sockaddr*)&a,&n)==0 && port) *port=(int32_t)ntohs(a.sin_port); } return r<0 ? -1 : 0; }\n",
    );
    out.push_str(
