        ...
        let raw_c = {
            ...
            if !out.status.success() {
                ...
                let first = err
                    .lines()
                    .next()
                    .unwrap_or("cgen failed")
                    .chars()
                    .take(80)
                    .collect::<String>();
                failures.push(format!(
                    "{stem}: cgen failed: {first} (exit {:?})",
                    out.status.code()
                ));
                continue;
            }
...
fn docs_headline_claims_are_recorded_at_named_surfaces() {
    ...
    for (surface, text, needle) in [
        ...
        (
            "README.md",
            &readme,
            "Full workspace:** 308 passed / 0 failed",
        ),
...
/// User-TYPE composite return (M1-ABI): `FUNCTION PT Make()` returns a user
/// composite struct. The interpreter collects `{fname}.*` slots after the
/// callee body; the Rust CEmitter discovers members via
/// `collect_composite_ret_members`, emits `xb_comp_PT` typedef + struct local
/// + fallback return; cgen.x mirrors via `scan_comp_ret$` / `##compMembers$`.
/// All three engines must produce `3\n4\n`.
#[test]
fn cemitter_and_cgen_agree_on_user_type_composite_return() {
    let tmp = std::env::temp_dir().join("xb_sync_comp_ret");
    fs::create_dir_all(&tmp).expect("mkdir");
    let cgen_exe = build_native_cgen(&tmp);
    ...
    let prog = FrontendUnit::parse(src)
        .expect("parse composite return program")
        .lower_ir()
        .expect("lower composite return program");
    let ir = TextIrEmitter::new().emit_program(&prog);

    let rust_c = CEmitter::new().emit_program(&prog);
    let rust_out = compile_and_exec(&tmp, "cret_rust", rust_c.as_bytes(), None);

    let self_c = cgen_emit(&cgen_exe, &ir);
    let self_out = compile_and_exec(&tmp, "cret_self", &self_c, None);

    let mut interp = Vec::new();
    Interpreter::new()
        .execute_main_with_input(&prog, Vec::new(), &mut interp)
        .expect("interpret composite return program");
    let interp_out: String = interp.into_iter().map(|l| format!("{l}\n")).collect();

    assert_eq!(interp_out, "3\n4\n", "composite return reference output");
    assert_eq!(
        rust_out, interp_out,
        "CEmitter failed to return user-TYPE composite"
    );
    assert_eq!(
        self_out, interp_out,
        "cgen.x failed to return user-TYPE composite"
    );
    let _ = fs::remove_dir_all(&tmp);
}
...
#[test]
fn cemitter_and_cgen_agree_on_byval_to_descriptor_redim_second_position() {
    // Positional twin of the descriptor-copy lock: the array sits at
    // callee position 1 behind a scalar. Guards `param_descriptor.get(i)`
    // (Rust) / `is_desc_position$(fn, ##curCallArg)` (cgen.x) against
    // position-0 hardcoding or 0/1-based confusion.
    let tmp = std::env::temp_dir().join("xb_sync_byval_desc_pos1");
    fs::create_dir_all(&tmp).expect("mkdir");
    let cgen_exe = build_native_cgen(&tmp);
    ...
    let src = "VERSION \"0.1\"\n\
               ...
               END FUNCTION\n";
    let prog = FrontendUnit::parse(src)
        .expect("parse byval_desc_pos1 program")
        .lower_ir()
        .expect("lower byval_desc_pos1 program");
    let ir = TextIrEmitter::new().emit_program_with_facets(&prog);

    let rust_c = CEmitter::new().emit_program(&prog);
    let rust_out = compile_and_exec(&tmp, "byval_desc_pos1_rust", rust_c.as_bytes(), None);

    let self_c = cgen_emit(&cgen_exe, &ir);
    let self_out = compile_and_exec(&tmp, "byval_desc_pos1_self", &self_c, None);

    let mut interp = Vec::new();
    Interpreter::new()
        .execute_main_with_input(&prog, Vec::new(), &mut interp)
        .expect("interpret byval_desc_pos1 program");
    let interp_out: String = interp.into_iter().map(|l| format!("{l}\n")).collect();

    assert_eq!(
        interp_out, "123\n5\n3\n1\n",
        "byval_desc_pos1 reference output"
    );
    assert_eq!(
        rust_out, interp_out,
        "CEmitter mishandled by-value REDIM copy at position 1"
    );
    assert_eq!(
        self_out, interp_out,
        "cgen.x mishandled by-value REDIM copy at position 1"
    );
    let _ = fs::remove_dir_all(&tmp);
}
...
#[test]
fn cemitter_and_cgen_agree_on_byval_string_descriptor_chain_copy() {
    // String twin of the descriptor-chain lock: descriptor-owned string
    // array passed by value to a REDIM callee. Exercises the chain fix
    // (facet-dual gate + copy pair) crossed with the `&(char**)` cell
    // shape and the et=2 deep-copy path.
    let tmp = std::env::temp_dir().join("xb_sync_byval_str_desc_chain");
    fs::create_dir_all(&tmp).expect("mkdir");
    let cgen_exe = build_native_cgen(&tmp);
    ...
    let src = "VERSION \"0.1\"\n\
               ...
               PRINT UBOUND(m$[])\n\
               END FUNCTION\n\
               FUNCTION Leaf (STRING w$[])\n\
               REDIM w$[5]\n\
               w$[0] = \"zz\"\n\
               PRINT UBOUND(w$[])\n\
               END FUNCTION\n";
    let prog = FrontendUnit::parse(src)
        .expect("parse byval_str_desc_chain program")
        .lower_ir()
        .expect("lower byval_str_desc_chain program");
    let ir = TextIrEmitter::new().emit_program_with_facets(&prog);

    let rust_c = CEmitter::new().emit_program(&prog);
    let rust_out = compile_and_exec(&tmp, "byval_str_desc_chain_rust", rust_c.as_bytes(), None);

    let self_c = cgen_emit(&cgen_exe, &ir);
    let self_out = compile_and_exec(&tmp, "byval_str_desc_chain_self", &self_c, None);

    let mut interp = Vec::new();
    Interpreter::new()
        .execute_main_with_input(&prog, Vec::new(), &mut interp)
        .expect("interpret byval_str_desc_chain program");
    let interp_out: String = interp.into_iter().map(|l| format!("{l}\n")).collect();

    assert_eq!(
        interp_out, "5\n1\n1\n",
        "byval_str_desc_chain reference output"
    );
    assert_eq!(
        rust_out, interp_out,
        "CEmitter mishandled chained by-value string REDIM copy"
    );
    assert_eq!(
        self_out, interp_out,
        "cgen.x mishandled chained by-value string REDIM copy"
    );
    let _ = fs::remove_dir_all(&tmp);
}
...
#[test]
fn cemitter_and_cgen_agree_on_byval_empty_descriptor_split() {
    // Empty-array by-value copy, pinned as a DOCUMENTED SPLIT: the copy
    // machinery itself agrees everywhere (NULL data + ub cell, REDIM
    // from empty works via realloc-NULL, caller isolated), but
    // UBOUND-of-empty reports raw -1 in both C backends where the
    // interpreter reports 0 (pre-existing UBOUND-empty semantic split,
    // orthogonal to the copy arm, which passes the ub through faithfully).
    let tmp = std::env::temp_dir().join("xb_sync_byval_desc_empty");
    fs::create_dir_all(&tmp).expect("mkdir");
    let cgen_exe = build_native_cgen(&tmp);
    ...
    let src = "VERSION \"0.1\"\n\
               ...
               END FUNCTION\n";
    let prog = FrontendUnit::parse(src)
        .expect("parse byval_desc_empty program")
        .lower_ir()
        .expect("lower byval_desc_empty program");
    let ir = TextIrEmitter::new().emit_program_with_facets(&prog);

    let rust_c = CEmitter::new().emit_program(&prog);
    let rust_out = compile_and_exec(&tmp, "byval_desc_empty_rust", rust_c.as_bytes(), None);

    let self_c = cgen_emit(&cgen_exe, &ir);
    let self_out = compile_and_exec(&tmp, "byval_desc_empty_self", &self_c, None);

    let mut interp = Vec::new();
    Interpreter::new()
        .execute_main_with_input(&prog, Vec::new(), &mut interp)
        .expect("interpret byval_desc_empty program");
    let interp_out: String = interp.into_iter().map(|l| format!("{l}\n")).collect();

    assert_eq!(interp_out, "0\n2\n0\n", "byval_desc_empty reference output");
    assert_eq!(
        rust_out, "-1\n2\n-1\n",
        "CEmitter empty-copy behavior changed"
    );
    assert_eq!(
        self_out, "-1\n2\n-1\n",
        "cgen.x empty-copy behavior changed"
    );
}
...
#[test]
fn cemitter_and_cgen_agree_on_byval_shared_to_plain_copy() {
    // Shared-dual source by value to a PLAIN callee that writes:
    // the copy arm must copy from the `_arr` facet cells (bare copy,
    // plain position), isolating the shared caller storage.
    let tmp = std::env::temp_dir().join("xb_sync_byval_shared_plain");
    fs::create_dir_all(&tmp).expect("mkdir");
    let cgen_exe = build_native_cgen(&tmp);
    ...
    let src = "VERSION \"0.1\"\n\
               ...
               END FUNCTION\n";
    let prog = FrontendUnit::parse(src)
        .expect("parse byval_shared_plain program")
        .lower_ir()
        .expect("lower byval_shared_plain program");
    let ir = TextIrEmitter::new().emit_program_with_facets(&prog);

    let rust_c = CEmitter::new().emit_program(&prog);
    let rust_out = compile_and_exec(&tmp, "byval_shared_plain_rust", rust_c.as_bytes(), None);

    let self_c = cgen_emit(&cgen_exe, &ir);
    let self_out = compile_and_exec(&tmp, "byval_shared_plain_self", &self_c, None);

    let mut interp = Vec::new();
    Interpreter::new()
        .execute_main_with_input(&prog, Vec::new(), &mut interp)
        .expect("interpret byval_shared_plain program");
    let interp_out: String = interp.into_iter().map(|l| format!("{l}\n")).collect();

    assert_eq!(
        interp_out, "99\n4\n3\n4\n",
        "byval_shared_plain reference output"
    );
    assert_eq!(
        rust_out, interp_out,
        "CEmitter mishandled shared by-value plain copy"
    );
    assert_eq!(
        self_out, interp_out,
        "cgen.x mishandled shared by-value plain copy"
    );
    let _ = fs::remove_dir_all(&tmp);
}

#[test]
fn cemitter_and_cgen_agree_on_byval_shared_string_to_plain_copy() {
    // Shared string-dual source by value to a PLAIN callee that
    // writes: deep copy (et=2) from the shared string facet cells,
    // caller storage isolated (`aa`/`bb` survive `zz`).
    let tmp = std::env::temp_dir().join("xb_sync_byval_shared_str_plain");
    fs::create_dir_all(&tmp).expect("mkdir");
    let cgen_exe = build_native_cgen(&tmp);
    ...
    let src = "VERSION \"0.1\"\n\
               ...
               END FUNCTION\n";
    let prog = FrontendUnit::parse(src)
        .expect("parse byval_shared_str_plain program")
        .lower_ir()
        .expect("lower byval_shared_str_plain program");
    let ir = TextIrEmitter::new().emit_program_with_facets(&prog);

    let rust_c = CEmitter::new().emit_program(&prog);
    let rust_out = compile_and_exec(&tmp, "byval_shared_str_plain_rust", rust_c.as_bytes(), None);

    let self_c = cgen_emit(&cgen_exe, &ir);
    let self_out = compile_and_exec(&tmp, "byval_shared_str_plain_self", &self_c, None);

    let mut interp = Vec::new();
    Interpreter::new()
        .execute_main_with_input(&prog, Vec::new(), &mut interp)
        .expect("interpret byval_shared_str_plain program");
    let interp_out: String = interp.into_iter().map(|l| format!("{l}\n")).collect();

    assert_eq!(
        interp_out, "zz\nbb\naa\nbb\n",
        "byval_shared_str_plain reference output"
    );
    assert_eq!(
        rust_out, interp_out,
        "CEmitter mishandled shared string by-value plain copy"
    );
    assert_eq!(
        self_out, interp_out,
        "cgen.x mishandled shared string by-value plain copy"
    );
    let _ = fs::remove_dir_all(&tmp);
}
